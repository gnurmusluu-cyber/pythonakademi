...
def egitim_ekrani(u, mufredat, msgs, emotions_module, ranks_module, ilerleme_fonksiyonu, normalize_fonksiyonu, supabase, inceleme_modu=False):
    # --- 0. SİBER-HUD VE KATMANLI YERLEŞİM CSS ---
    st.markdown('''
        <style>
        ...
        /* SABİT ÜST HUD BAR (Z-Index Optimize Edildi) */
        .cyber-hud {
            position: fixed; top: 0; left: 0; width: 100%;
            background: rgba(14, 17, 23, 0.98);
            border-bottom: 2px solid #00E5FF;
            z-index: 100; /* Sidebar butonlarını (1000+) kapatmaması için düşük tutuldu */
            padding: 10px 25px;
        ...
        /* Ana İçeriği HUD Barına Yaklaştırma (Boşluk Azaltıldı) */
    ...
    # 7.5 : 2.5 Bölünmüş Alan (Liderlik Tablosu Geri Geldi)
    ...
    with cl:
        ...
        if not st.session_state.cevap_dogru and st.session_state.error_count < 4:
            ...
            user_code = st.text_area("Siber-Editor", value=egz['sablon'], height=180, key=f"final_elite_{egz['id']}_{st.session_state.reset_trigger}", label_visibility="collapsed")
    ...
    with cr:
        # Liderlik Tablosu Sağ Kolona Geri Mühürlendi
