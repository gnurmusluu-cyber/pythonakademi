...
def egitim_ekrani(u, mufredat, msgs, emotions_module, ranks_module, ilerleme_fonksiyonu, normalize_fonksiyonu, supabase, inceleme_modu=False):
    # --- 0. SİBER-HUD VE DARALTILMIŞ BOŞLUK CSS ---
    st.markdown('''
        <style>
        /* Sidebar ile Ana İçerik Arasındaki Boşluğu Sıfırlama */
        .block-container {
    ...
    # --- 2. YAN PANEL (SIDEBAR) GERİ GELDİ ---
    with st.sidebar:
        # Liderlik tablosu burada gösterildiği için sidebar otomatik açılır
    ...
    with col_p1:
        st.markdown(f"<div style='color:#00E5FF; font-size:0.8rem; font-weight:bold;'>🚀 AKADEMİ: %{int((m_idx/total_m)*100)}</div>", unsafe_allow_html=True)
        st.progress(min((m_idx) / total_m, 1.0))
    with col_p2:
        st.markdown(f"<div style='color:#00E5FF; font-size:0.8rem; font-weight:bold;'>📍 MODÜL {m_idx + 1} - GÖREV {c_i}/{t_i}</div>", unsafe_allow_html=True)
        st.progress(c_i / t_i)
    ...
    if not st.session_state.cevap_dogru and st.session_state.error_count < 4:
        ...
        user_code = st.text_area("Siber-Editor", value=egz['sablon'], height=180, key=f"fix_v1_{egz['id']}_{st.session_state.reset_trigger}", label_visibility="collapsed")
