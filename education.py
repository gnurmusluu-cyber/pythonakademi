...
def egitim_ekrani(u, mufredat, msgs, emotions_module, ranks_module, ilerleme_fonksiyonu, normalize_fonksiyonu, supabase):
    # --- ANIMASYON TOGGLE (A/B) PROTOKOLÜ ---
    ...
    # --- 0. SİBER-GÖRSEL ZIRH (GÖRÜNÜRLÜK, MOBİL UYUM VE OKUNABİLİRLİK MÜHRÜ) ---
    st.markdown(f'''
        <style>
        ...
        /* ANA İÇERİK BOŞLUĞU (BAŞLIK VE HUD ÇAKIŞMA ÖNLEYİCİ) */
        [data-testid="stMainViewContainer"] {{
            padding-top: 195px !important; /* Başlığı HUD'ın altından kurtarır */
        }}

        /* SABİT ÜST HUD BAR */
        .cyber-hud {{
            position: fixed; top: 0; left: 0; right: 0;
            height: 125px; background-color: #0e1117 !important;
        ...
        /* PİTO KOKPİT GÖRSELİ (85px) */
        .hud-pito-gif img {{
            width: 85px; height: 85px; border-radius: 50%; border: 3px solid #00E5FF;
            object-fit: cover; background: #000; margin-right: 15px;
            box-shadow: 0 0 15px #00E5FF;
        }}

        /* --- SİBER-VURGU ANİMASYONLARI (DOUBLE PULSE) --- */
        @keyframes cyberPulseA {{
            0% {{ transform: scale(1); color: #00E5FF; text-shadow: none; }}
            50% {{ transform: scale(1.8); color: #FF0000; text-shadow: 0 0 20px #FF0000, 0 0 50px #FF0000; }}
        ...
        @media (max-width: 768px) {{
            .cyber-hud {{ height: 180px !important; flex-direction: column; justify-content: center; padding: 10px; }}
            .hud-pito-gif img {{ width: 70px !important; height: 70px !important; margin-right: 0; margin-bottom: 5px; }}
            .hud-item {{ font-size: 0.85rem !important; margin: 3px 5px !important; }}
            [data-testid="stMainViewContainer"] {{ padding-top: 275px !important; }} 
        }}
    ...
    with cl:
        ...
        if not st.session_state.cevap_dogru and e_count < 4:
            if e_count > 0:
                ...
                if e_count == 3: st.warning(f"💡 **İpucu:** {egz.get('ipucu', '... ')}")
