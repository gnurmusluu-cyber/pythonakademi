...
def egitim_ekrani(u, mufredat, msgs, emotions_module, ranks_module, ilerleme_fonksiyonu, normalize_fonksiyonu, supabase):
    ...
    try:
        ...
        egz = next((e for e in modul["egzersizler"] if e["id"] == egz_id), modul["egzersizler"][0]) [cite: 2026-02-07]
    except Exception:
        st.error("🚨 Veri Okuma Hatası! Siber-müfredat yüklenemedi.")
        return
    ...
    def kod_calistir_cikti_al(kod, giris_verisi=""):
        ...
        try:
            ...
            return res if res else "ℹ️ Bu kod herhangi bir çıktı üretmedi." [cite: 2026-02-07]
        ...
        if st.button("KODU KONTROL ET 🚀", type="primary", use_container_width=True):
            if has_input and not st.session_state.get("user_input_val", "").strip():
                ...
                if normalize_fonksiyonu(u_code) == normalize_fonksiyonu(egz["dogru_cevap_kodu"]):
                    yeni_xp = int(u.get("toplam_puan", 0)) + max(0, 20-(e_count*5)) [cite: 2026-02-07]
