...
def egitim_ekrani(u, mufredat, msgs, emotions_module, ranks_module, ilerleme_fonksiyonu, normalize_fonksiyonu, supabase):
    ...
    with main_col:
        ...
        if not st.session_state.cevap_dogru and st.session_state.error_count < 4:
            ...
            user_code = st.text_area("Siber-Editor", value=egz['sablon'], height=180, 
                                     key=f"final_elite_{egz['id']}_{st.session_state.reset_trigger}", label_visibility="collapsed")
            ...
            with b_run:
                if st.button("KODU KONTROL ET 🚀", type="primary", use_container_width=True):
                    ...
                    if normalize_fonksiyonu(user_code) == normalize_fonksiyonu(egz['dogru_cevap_kodu']):
                        ...
                        st.balloons() # BALONLAR SADECE BURADA
            ...
            st.warning("🚨 Pito'nun ideal çözümü ve çıktısı:")
    ...
    with side_col:
        # 'Onur Kürsüsü' başlığı ranks_module içinden geldiği için burada sildik
