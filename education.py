...
def egitim_ekrani(u, mufredat, msgs, emotions_module, ranks_module, ilerleme_fonksiyonu, normalize_fonksiyonu, supabase):
    # --- 1. VERİ YAPISI VE GÜVENLİK KONTROLÜ ---
    # Müfredatın liste mi yoksa sözlük mü olduğunu kontrol et ve m_list'i mühürle
    if isinstance(mufredat, dict) and "pito_akademi_mufredat" in mufredat:
        m_list = mufredat["pito_akademi_mufredat"]
    else:
        m_list = mufredat

    # Mevcut modül ve egzersiz verilerini al
    try:
        m_idx = int(u['mevcut_modul']) - 1
        modul = m_list[m_idx]
        # 🚨 KRİTİK DÜZELTME: modul tanımlandıktan sonra egz tanımlanıyor
        egz = next((e for e in modul['egzersizler'] if e['id'] == str(u['mevcut_egzersiz'])), modul['egzersizler'][0])
    except (IndexError, KeyError, TypeError):
        st.error("🚨 SİBER-HATA: Müfredat verisi veya kullanıcı ilerlemesi okunamadı!")
        return
    ...
    st.markdown(f'''
        ...
        .cyber-hud {{
            position: fixed; top: 0; left: 0; right: 0; height: 145px;
    ...
    with cl:
        ...
        if st.button("KODU KONTROL ET 🚀", type="primary", use_container_width=True):
            if has_input and not st.session_state.get('user_input_val', '').strip():
                st.error("🚨 HATA: Veri girişi bekliyor!")
