...
def egitim_ekrani(u, mufredat, msgs, emotions_module, ranks_module, ilerleme_fonksiyonu, normalize_fonksiyonu, supabase, inceleme_modu=False):
    # --- 0. SİBER-HUD VE ELECTRIC BLUE CSS MÜHRÜ ---
    st.markdown('''
        <style>
        .stApp { background-color: #0e1117; }
        
        /* SABİT ÜST BAR (CYBER-HUD) */
        .cyber-hud {
            position: fixed;
            top: 0;
            left: 0;
        ...
        .hud-value { color: #00E5FF; font-weight: bold; }

        /* İçeriğin HUD altında kalmaması için boşluk */
    ...
    # --- 1. SABİT SİBER-HUD ÇIKTISI ---
    p_xp = max(0, 20 - (st.session_state.error_count * 5))
    st.markdown(f'''
        <div class="cyber-hud">
            <div class="hud-item">👤 Yazılımcı: <span class="hud-value">{u['ad_soyad']}</span></div>
            <div class="hud-item">💎 Potansiyel: <span class="hud-value">{p_xp} XP</span></div>
            <div class="hud-item">⚠️ Hata: <span class="hud-value">{st.session_state.error_count}/4</span></div>
            <div class="hud-item">🏆 Toplam: <span class="hud-value">{int(u['toplam_puan'])} XP</span></div>
        </div>
    ''', unsafe_allow_html=True)

    # --- 2. ANA İÇERİK BAŞLANGICI ---
    ...
    with main_col:
        ...
        if inceleme_modu:
            ...
            user_code = st.text_area("Siber-Editor", value=egz['sablon'], height=180, 
                                     key=f"hud_ed_{egz['id']}_{st.session_state.reset_trigger}", label_visibility="collapsed")
    ...
    st.markdown('</div>', unsafe_allow_html=True) # Main content bitişi
