...
with col_main:
    ...
    if not st.session_state.exercise_passed and st.session_state.fail_count < 4:
        # ÖNEMLİ: Ace Editor'deki metni anlık olarak backend'e çekmek için özel senkronizasyon anahtarı.
        code = st_ace(value=curr_ex.get('task',''), language="python", theme="monokai", font_size=18, height=220, auto_update=True, key=f"ace_vX_{curr_idx}")
        
        if st.button("🔍 Kodumu Kontrol Et", use_container_width=True):
            # ADIM 1: Placeholder/Boşluk Kontrolü (Hata Hakkı Gitmez)
            if "___" in code:
            ...
            try:
                ...
                # BAŞARI KOŞULU (SADECE BURADA BAŞARI TETİKLENİR)
                if curr_ex.get('check', lambda c,o,i: True)(code, out, mock_env):
                    ...
                    raise Exception("Output mismatch") # Mantık hatası (kod çalıştı ama sonuç yanlış)
