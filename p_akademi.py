...
st.markdown("""
    ...
    .success-msg { color: #2E7D32; font-weight: bold; font-size: 20px; }
    </style>
    """, unsafe_allow_html=True)

# --- 2. VERİ VE RÜTBE MANTIĞI ---
# Google Sheets URL (CSV formatında dışa aktarım)
...
def render_gif(name):
    try:
        with open(f"assets/{name}.gif", "rb") as f:
            ...
            st.markdown(f'<img src="data:image/gif;base64,{url}" width="300">', unsafe_allow_html=True)
    except:
        st.info(f"[{name}.gif yüklenemedi]")

# --- 3. EKSİKSİZ MÜFREDAT (8 MODÜL / 40 ADIM) ---
training_data = [
    {"module_title": "1. İletişim: print() ve Çıktı Dünyası", "exercises": [
        ...
        {"msg": "Virgül (`,`) farklı veri tiplerini aynı satırda birleştirir. 'Puan:' metni ile **100** sayısını yanyana bas.", "task": "print('Puan:', ___)", "solution": "print('Puan:', 100)", "hint": "Virgülden sonra tırnaksız 100 yaz."},
        {"msg": "`#` işareti Python'a 'Bu satırı görmezden gel' demektir. Satırın en başına **#** işaretini koy.", "task": "___ bu bir yorumdur", "solution": "# bu bir yorumdur", "hint": "Kare (diyez) işaretini en başa koy."},
        {"msg": "`\\n` kaçış karakteri metni alt satıra böler. Boşluğa **\\n** yaz.", "task": "print('Üst' + '___' + 'Alt')", "solution": "print('Üst\\nAlt')", "hint": "Tırnaklar içine \\n karakterini yazmalısın."}
    ]},
    {"module_title": "2. Hafıza: Değişkenler ve input()", "exercises": [
        {"msg": "Değişkenler hafızadaki kutulardır. `=` işareti atama yapar. `yas` değişkenine **15** değerini ata.", "task": "yas = ___", "solution": "yas = 15", "hint": "yas = 15 şeklinde yaz."},
    ...
    {"module_title": "3. Karar Yapıları: If-Else Dünyası", "exercises": [
        {"msg": "Eşitlik kontrolü için `==` kullanılır. Sayı 10'a eşitse kontrolü için **==** yaz.", "task": "if 10 ___ 10: print('OK')", "solution": "if 10 == 10:", "hint": "Çift eşittir kullan."},
        {"msg": "Şart yanlışsa `else:` bloğu çalışır. Boşluğa **else** yaz.", "task": "if 5 > 10: pass\n___: print('Hata')", "solution": "else:", "hint": "Sadece else: yaz."},
    ...
    {"module_title": "4. Otomasyon: For ve While Döngüleri", "exercises": [
        ...
        {"msg": "`break` döngüyü bitirir. i değeri 1 olduğunda döngüyü bitiren **break** komutunu yaz.", "task": "for i in range(5):\n if i == 1: ___\n print(i)", "solution": "break", "hint": "break yaz."},
        {"msg": "`continue` o adımı atlar. 1 değerini atlayan **continue** komutunu yaz.", "task": "for i in range(3):\n if i == 1: ___\n print(i)", "solution": "continue", "hint": "continue yaz."},
    ...
    {"module_title": "8. Kalıcılık: Dosya Yönetimi", "exercises": [
        ...
        {"msg": "`.write()` metodu veriyi yazar. Boşluğa **write** yaz.", "task": "f = open('t.txt', 'w')\ f.___('X')\ f.close()", "solution": "f.write('X')", "hint": "write yaz."},
...
def show_leaderboard():
    try:
        ...
        st.write("### 🎖️ Liderlik Tablosu")
        for _, row in df.sort_values(by="Puan", ascending=False).head(10).iterrows():
            st.markdown(f"""<div class="leaderboard-card"><b>{row['Öğrencinin Adı']}</b><br>{row['Rütbe']} | {row['Puan']} Puan</div>""", unsafe_allow_html=True)
    except:
        st.info("Liderlik tablosu güncelleniyor...")
...
if st.session_state.user is None:
    ...
    with col_l:
        ...
        if okul_no:
            # Buraya DB okuma ve kayıt mantığı eklenebilir. 
            # Örnek başlangıç verisi:
            st.session_state.user = {"Okul No": okul_no, "Ad": "Öğrenci", "Mevcut Modül": 1, "Mevcut Egzersiz": 1, "Puan": 0, "Sınıf": "9-A"}
            st.rerun()
    with col_r:
        show_leaderboard()
    ...
    st.write(f"📍 **Modül {m_idx+1}:** {training_data[m_idx]['module_title']} | **Adım:** {e_idx+1}/5")

    c_main, c_side = st.columns([2.5, 1])

    with c_main:
        if st.session_state.is_completed: render_gif("pito_basari")
        ...
        # Doğru Cevap/Hata Durumu Sonrası "Sonraki Adım"
        if st.session_state.is_completed:
            st.divider()
            if st.session_state.errors >= 4:
                st.error(f"🚨 4 hata yaptın. Çözüm: {curr_ex['solution']}")
            else:
