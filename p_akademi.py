...
training_data = [
    ...
    # Diğer 4 modül (Listeler, Fonksiyonlar, OOP, Dosya) baseline yapısıyla devam eder...
]
...
with col_main:
    ...
    # --- KRİTİK: 3 HATA SONRASI ÇÖZÜM GÖSTERİMİ ---
        ...
        if st.button("🔍 Kontrol Et", use_container_width=True):
            ...
            if out.startswith("⚠️"): st.error(out)
                ...
                if st.session_state.fail_count >= 4:
                    st.error("❌ Maalesef son hakkın da bitti. Bu egzersizden puan kazanamadın ama devam edebilirsin.")
                    st.session_state.exercise_passed = True
                else: st.error(f"Hatalı! {st.session_state.fail_count}. deneme başarısız. " + ("Çözüm açıldı!" if st.session_state.fail_count==3 else ""))
            else:
