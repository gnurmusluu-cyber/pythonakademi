...
# --- 4. LİDERLİK TABLOSU (MÜHÜRLÜ - ST.STOP ÖNCESİ) ---
...
with col_app:
    ...
    if st.session_state.db_module >= 8 and not st.session_state.graduation_view:
        st.session_state.graduation_view = True; st.balloons()
    ...
    if st.session_state.graduation_view:
        ...
        with c1:
            if st.button("🔄 Eğitimi Tekrar Al (Puan Sıfırlanır)"):
                st.session_state.update({'db_module': 0, 'db_exercise': 0, 'current_module': 0, 'current_exercise': 0, 'total_score': 0, 'completed_modules': [False]*8, 'graduation_view': False, 'scored_exercises': set(), 'last_output': "", 'feedback_msg': ""})
                force_save(); st.rerun()
        with c2:
            if st.button("🔍 İnceleme Modu ve Liderlikte Kal"):
                st.session_state.update({'current_module': 0, 'current_exercise': 0, 'graduation_view': False, 'last_output': "", 'feedback_msg': ""}); st.rerun()
        st.stop()

    # --- 6. EKSİKSİZ VE MÜKEMMEL PEDAGOJİK MÜFREDAT (40 ADIM) ---
    training_data = [
        {"module_title": "1. İletişim: print() ve Metin Dünyası", "exercises": [
            {"msg": "**Pito'nun Notu:** Python'ın dünyayla konuşma yolu `print()` fonksiyonudur. Ekrana yazacağın metinleri mutlaka tırnak (' ') içine almalısın. Tırnaklar Python'a 'buradaki ifadeyi olduğu gibi yansıt' komutunu verir. Bu, programın kullanıcıyla ilk temasıdır.\n\n**GÖREV:** Editor içine tam olarak **'Merhaba Pito'** metnini tırnaklar içerisinde yaz ve kontrol et!", "task": "print('___')", "check": lambda c, o, i: "Merhaba Pito" in o, "solution": "print('Merhaba Pito')", "hint": "Metnin başına ve sonuna tek (') tırnak koyduğundan emin ol."},
            {"msg": "**Pito'un Notu:** Sayılar (Integer), metinlerden farklıdır; tırnak gerektirmezler. Eğer bir sayıya tırnak koyarsan Python onu sayı değil, bir 'yazı' olarak görür ve üzerinde matematiksel işlemler yapamaz.\n\n**GÖREV:** Boşluğa tırnak kullanmadan sadece **100** sayısını yaz. Bu sayede Python onun bir sayı olduğunu anlayacaktır.", "task": "print(___)", "check": lambda c, o, i: "100" in o, "solution": "print(100)", "hint": "Rakamları tırnaksız yazmalısın. Eğer '100' yazarsan bu bir metin (String) olur!"},
            {"msg": "**Pito'nun Notu:** Virgül (`,`) farklı veri tiplerini (örneğin bir metin ve bir sayı) aynı satırda birleştirir ve araya otomatik bir boşluk koyar. Bu, değişkenleri ve mesajları birleştirmek için en profesyonel yöntemdir.\n\n**GÖREV:** 'Puan:' metni ile **100** sayısını yan yana basmak için virgülden sonraki boşluğa sadece **100** yaz.", "task": "print('Puan:', ___)", "check": lambda c, o, i: "100" in o, "solution": "print('Puan:', 100)", "hint": "Virgülden sonra sadece sayısal değeri (100) yazmalısın."},
        ...
        {"module_title": "2. Hafıza: Değişkenler ve Atama", "exercises": [
            {"msg": "**Pito'nun Notu:** Değişkenler bellekteki (RAM) kutulardır. `=` işareti bir 'atama operatörü'dür ve sağdaki değeri soldaki kutunun içine koyar. Artık o kutuya ismiyle seslendiğinde içindeki değeri sana verir.\n\n**GÖREV:** `yas` ismindeki hafıza kutusuna sayısal olarak **15** değerini ata.", "task": "yas = ___", "check": lambda c, o, i: "15" in str(i.get('yas', '')), "solution": "yas = 15", "hint": "Eşittir işaretinden sonra sadece değeri (15) yazmalısın."},
            {"msg": "**Pito'nun Notu:** Metin (String) verilerini hafızaya alırken tırnak şarttır. Python büyük-küçük harfe duyarlıdır!\n\n**GÖREV:** `isim` değişkenine **'Pito'** metnini ata. Metni tırnak içinde yazmayı unutma!", "task": "isim = '___'", "check": lambda c, o, i: "Pito" in str(i.get('isim', '')), "solution": "isim = 'Pito'", "hint": "Metni tırnaklar içerisine tam olarak Pito şeklinde yaz."},
        ...
        {"module_title": "8. Kalıcılık: Dosya Yönetimi", "exercises": [
            ...
            {"msg": "**Pito'un Notu:** Okuma modunda (**'r'**) dosyayı sadece görebiliriz, değiştiremeyiz.\n\n**GÖREV:** Dosyayı sadece okuma modunda açmak için boşluğa **r** koy.", "task": "f = open('t.txt', '___')", "check": lambda c, o, i: "r" in c, "solution": "f = open('t.txt', 'r')", "hint": "r koy."},
    ...
    with c_box_m:
        ...
        st.caption(f"Adım: {st.session_state.current_exercise + 1}/5 | {'🔒 İnceleme Modu' if is_review_mode else f'🎁 Potansiyel: {st.session_state.current_potential_score} | ❌ Hata: {st.session_state.fail_count}/4'}")
    ...
    if not is_review_mode and st.session_state.fail_count < 4 and not st.session_state.exercise_passed:
        ...
        if "input" in curr_ex['solution']:
            ...
            custom_input = st.text_input("📝 Girdi Kutusu:", placeholder="Örn: 10 veya Pito", key=f"inp_{st.session_state.current_module}_{st.session_state.current_exercise}").strip()
            ...
            if "___" in code: st.session_state.feedback_msg = "⚠️ Pito bekliyor: Lütfen önce boşluğu doldur!"; st.rerun()
                ...
                try:
                    ...
                    if curr_ex.get('check', lambda c, o, i: True)(code, out, mock_env):
                        st.session_state.update({'feedback_msg': "✅ Harika! Görevi başarıyla mühürledin.", 'last_output': out, 'exercise_passed': True})
        ...
        with cn:
            if st.session_state.current_exercise < 4:
                ...
                if st.button("🏆 Modülü Bitir"):
                    ...
                    st.session_state.update({'current_module': st.session_state.current_module + 1, 'current_exercise': 0, 'exercise_passed': False, 'fail_count': 0, 'current_potential_score': 20, 'feedback_msg': "", 'last_output': ""}); st.rerun()
            elif st.session_state.current_module == 7:
