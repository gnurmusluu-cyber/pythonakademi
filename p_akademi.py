...
training_data = [
    {"module_title": "1. Giriş ve Çıktı", "exercises": [
        ...
        {"msg": "Python'da sayılar metinlerden farklıdır. Matematiksel bir değer olan sayıları ekrana yazdırırken **tırnak işareti kullanmayız.** Şimdi ekrana 100 sayısını yazdır.", "task": "print(___)", "check": lambda c, o: "100" in o, "solution": "print(100)"},
    ...
    {"module_title": "2. Değişkenler", "exercises": [
        {"msg": "Değişkenler bilgi saklamaya yarar. yas = 15 yazarak bir tam sayı değişkeni oluştur ve yazdır.", "task": "yas = ___\nprint(yas)", "check": lambda c, o: "15" in o, "solution": "yas = 15\nprint(yas)"},
        ...
        {"msg": "int() ile kullanıcı girişlerini tam sayıya çevirmelisin.", "task": "n = ___(___('S: '))\nprint(n + 1)", "check": lambda c, o: "int" in c, "solution": "n = int(input('10'))\nprint(n + 1)"}
    ]},
    ...
    {"module_title": "6. Fonksiyonlar ve Gelişmiş Veriler", "exercises": [
        {"msg": "**Fonksiyonlar**, belirli bir görevi yerine getiren ve tekrar tekrar kullanılabilen kod bloklarıdır. 'def' kelimesiyle tanımlanır. 'f' adında bir fonksiyon oluştur ve içine print('X') yaz.", "task": "___ f(): print('X')", "check": lambda c, o: "def" in c, "solution": "def f(): print('X')"},
        {"msg": "**Tuple (Demet)**, listelere çok benzer ancak en büyük farkı **değiştirilemez** olmasıdır. Oluşturulduktan sonra içindeki veri 'kilitlenir'. Listeler için [] kullanılırken, Demetler için **()** kullanılır. (1, 2) demeti oluştur.", "task": "t = (___, 2)\nprint(t)", "check": lambda c, o: "1" in c, "solution": "t = (1, 2)\nprint(t)"},
        {"msg": "**Sözlükler (Dictionary)**, verileri 'Anahtar: Değer' (Key: Value) çiftleri şeklinde tutar. Tıpkı bir İngilizce sözlükte kelime (anahtar) ve anlamı (değer) gibi. 'ad' anahtarına 'Pito' değerini ata.", "task": "d = {'ad': '___'}\nprint(d['ad'])", "check": lambda c, o: "Pito" in c, "solution": "d = {'ad': 'Pito'}\nprint(d['ad'])"},
        {"msg": "Sözlükteki tüm başlıkları (anahtarları) görmek için **keys()** metodunu kullanırız. d sözlüğündeki anahtarları yazdır.", "task": "d={'a':1}\nprint(d.___())", "check": lambda c, o: "keys" in c, "solution": "d={'a':1}\nprint(d.keys())"},
        {"msg": "**Set (Küme)**, her elemandan sadece bir adet barındıran benzersiz bir koleksiyondur. İçine aynı veriyi iki kez koysanız bile o sadece birini tutar. {1, 2, 1} yazarak bir küme oluştur.", "task": "s = {1, 2, ___}\nprint(s)", "check": lambda c, o: "1" in c, "solution": "s = {1, 2, 1}\nprint(s)"}
    ]},
    {"module_title": "7. Nesne Tabanlı Programlama (OOP)", "exercises": [
        {"msg": "**Sınıflar (Class)**, nesnelerin nasıl oluşturulacağını gösteren birer 'taslaktır'. Örneğin; bir robotun hangi parçalara sahip olacağını belirleyen proje çizimi bir sınıftır. Sınıf oluştururken **class** (küçük harf!) kullanılır.", "task": "___ Robot: pass", "check": lambda c, o: "class" in c, "solution": "class Robot: pass"},
        {"msg": "**Nesneler**, sınıftan (taslaktan) üretilen gerçek örneklerdir. Sınıf bir proje çizimiyse, nesne o çizimle üretilen gerçek robottur. R sınıfından 'p' nesnesi üret.", "task": "class R: pass\np = ___()", "check": lambda c, o: "R()" in c, "solution": "class R: pass\np = R()"},
        {"msg": "Nesnelerin içindeki verilere **Nitelik (Attribute)** denir. Nesne adından sonra nokta '.' koyarak niteliklere erişir ve değer atarız. Robota 'renk' niteliği olarak 'Mavi' ata.", "task": "class R: pass\np=R()\np.___ = 'Mavi'\nprint(p.renk)", "check": lambda c, o: "renk" in c, "solution": "class R: pass\np=R()\np.renk = 'Mavi'\nprint(p.renk)"},
        {"msg": "**Metotlar**, sınıfın içine yazılan fonksiyonlardır ve nesnenin yapabildiği işleri temsil eder. Sınıf içindeki metotların ilk parametresi her zaman **self** olmalıdır; bu sayede metot o nesnenin bilgilerine ulaşır.", "task": "class R:\n def ___(self):\n  print('Bip!')", "check": lambda c, o: "ses" in c, "solution": "class R:\n def ses(self):\n  print('Bip!')"},
        {"msg": "Bir metodu çalıştırmak için **nesne_adi.metot_adi()** formülünü kullanırız. 'r' nesnesi üzerinden 's' metodunu çağır.", "task": "class R: def s(self): print('X')\nr=R()\nr.___()", "check": lambda c, o: "s()" in c, "solution": "class R: def s(self): print('X')\nr=R()\nr.s()"}
    ]},
    {"module_title": "8. Dosya Yönetimi", "exercises": [
        {"msg": "Bilgisayardaki dosyalara ulaşmak için **open()** fonksiyonunu kullanırız. İlk parametre dosya adı, ikincisi ise 'kip'tir (mode). **'w' (write)** kipi, dosyayı **yazmak** için açar. Dosya yoksa oluşturur, varsa içini tamamen siler.", "task": "dosya = ___('n.txt', '___')", "check": lambda c, o: "open" in c and "'w'" in c, "solution": "dosya = open('n.txt', 'w')\nprint('Yazma modunda açıldı.')"},
        {"msg": "**write()** metodu, açılan dosyanın içine yazı yazmamızı sağlar. Dosyanın içine 'Pito' yazdır.", "task": "f = open('t.txt', 'w')\nf.___('Pito')\nf.close()", "check": lambda c, o: "write" in c, "solution": "f = open('t.txt', 'w'); f.write('Pito'); f.close()"},
        {"msg": "**'r' (read)** kipi, dosyayı sadece **okumak** için açar. Bu modda dosya üzerinde değişiklik yapılamaz (yalnızca okunabilir). Eğer dosya yoksa Python hata verir. t.txt dosyasını 'r' kipiyle aç.", "task": "f = open('t.txt', '___')", "check": lambda c, o: "'r'" in c, "solution": "f = open('t.txt', 'r'); f.close()"},
        {"msg": "**read()** metodu, dosyanın içindeki tüm metni tek bir parça halinde bize getirir. Dosyayı oku ve print ile ekrana yazdır.", "task": "f = open('t.txt', 'r')\nprint(f.___())\nf.close()", "check": lambda c, o: "read" in c, "solution": "f = open('t.txt', 'w'); f.write('Pito Akademi'); f.close(); f = open('t.txt', 'r'); print(f.read()); f.close()"},
        {"msg": "Python'da açılan her dosya işimiz bittiğinde **close()** ile kapatılmalıdır. Bu, dosyanın kaydedilmesini sağlar ve bilgisayarın hafızasını boşaltır. Dosyayı kapat.", "task": "f = open('t.txt', 'r')\nf.___()", "check": lambda c, o: "close" in c, "solution": "f = open('t.txt', 'r'); f.close()"}
    ]}
]
