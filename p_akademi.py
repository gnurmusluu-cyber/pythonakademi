...
# --- 3. SESSION STATE (BÜTÜN ANAHTARLARIN BAŞLATILMASI) ---
...
training_data = [
    {"module_title": "1. Giriş ve Çıktı", "exercises": [
        ...
        {"msg": "**'Puan:'** ve **100** yazdır.", "task": "print('Puan:', ___)", "check": lambda c, o: "100" in o, "solution": "print('Puan:', 100)", "has_output": True},
        {"msg": "Bir **#** ile yorum satırı oluştur.", "task": "___ bu bir yorumdur", "check": lambda c, o: "#" in c, "solution": "# bu bir yorumdur", "has_output": True},
        {"msg": "**'Üst'** ve **'Alt'** yazdır.", "task": "print('Üst' + '___' + 'Alt')", "check": lambda c, o: "\n" in o, "solution": "print('Üst\\nAlt')", "has_output": True}
    ]},
    ...
    {"module_title": "4. Döngüler", "exercises": [
        ...
        {"msg": "Sayacı (**i**) yazdır.", "task": "for i in range(2): print(___)", "check": lambda c, o: "1" in o, "solution": "for i in range(2): print(i)", "has_output": True}
    ]},
...
with col_main:
    ...
    if st.session_state.db_module >= 8:
        ...
        with c1:
            if st.button("🔄 Eğitimi Tekrar Al (Puan Sıfırlanır)"):
                st.session_state.update({'db_module':0, 'db_exercise':0, 'total_score':0, 'current_module':0, 'current_exercise':0, 'completed_modules':[False]*8, 'scored_exercises':set(), 'celebrated':False, 'exercise_passed':False, 'fail_count':0})
                force_save(); st.rerun()
        with c2:
    ...
    if new_m_idx != st.session_state.current_module:
        st.session_state.update({'current_module': new_m_idx, 'current_exercise': 0, 'fail_count': 0, 'exercise_passed': False}); st.rerun()
    ...
    with c_msg:
        st.info(f"##### 🗣️ Pito:\n{curr_ex['msg']}")
        st.caption(f"Adım: {e_idx + 1}/5 | " + ("🔒 İnceleme" if is_locked else f"🎁 Puan: {st.session_state.current_potential_score} | ❌ Hata: {st.session_state.fail_count}/3"))

    # --- KRİTİK: ÇÖZÜM REHBERİ (3 HATA SONRASI) ---
        ...
        if st.button("🔍 Kontrol Et", use_container_width=True):
            ...
            if out.startswith("⚠️"): st.error(out)
                ...
                if st.session_state.fail_count >= 4:
                    st.error("❌ Hakların bitti. Bu sorudan puan alamadın ama geçebilirsin.")
                    st.session_state.exercise_passed = True
                else: st.error(f"Hatalı! {st.session_state.fail_count}. deneme.")
            else:
    ...
    with c_n:
        if st.session_state.exercise_passed or is_locked:
            if e_idx < 4:
                ...
                if st.button("🏆 Modülü Bitir"): st.session_state.update({'current_module': st.session_state.current_module + 1, 'current_exercise': 0, 'fail_count': 0, 'exercise_passed': False}); st.rerun()
