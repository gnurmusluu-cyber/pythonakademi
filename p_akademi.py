...
# --- 2. SESSION STATE (SİSTEM ANAYASASI VE DUYGU HAFIZASI) ---
...
st.markdown("""
    ...
    .quest-container {
        ...
        border-bottom: 5px solid #3a7bd5; text-align: center;
    }
    .quest-bar { height: 20px; background: #e2e8f0; border-radius: 12px; margin: 10px 0; overflow: hidden; }
    ...
    .pito-bubble {
        ...
        border-radius: 25px; padding: 30px; margin-bottom: 20px; color: #1e1e1e;
        font-weight: 500; font-size: 1.2rem; box-shadow: 0 8px 16px rgba(0,0,0,0.05); line-height: 1.8;
    }
    .pito-bubble:after { content: ''; position: absolute; bottom: -20px; left: 70px; border-width: 20px 20px 0; border-style: solid; border-color: #3a7bd5 transparent; }
    ...
    .stButton > button { width: 100%; border-radius: 15px; height: 4em; background: linear-gradient(45deg, #3a7bd5, #00d2ff) !important; color: white !important; font-weight: bold; border: none; font-size: 1.1rem; box-shadow: 0 4px 15px rgba(58, 123, 213, 0.3); }
    [data-testid="stTextInput"] { border: 2.5px solid #3a7bd5 !important; border-radius: 12px; }
    </style>
    """, unsafe_allow_html=True)

# --- 3. KESİN GIF ÇÖZÜMÜ: HTML INJECTION + BASE64 ---
def show_pito_gif(width=320):
    emotion_map = {
    ...
    # Dosya yolunu MacBook/Unix uyumlu Pathlib ile çözüyoruz
    ...
    if gif_path.exists():
        ...
        # Tarayıcı önbelleğini kırmak için rastgele bir ID ekleyerek HTML üzerinden basıyoruz
        t = time.time()
        st.markdown(f'<div style="display: flex; justify-content: center; margin-bottom: 15px;"><img src="data:image/gif;base64,{b64}?t={t}" width="{width}" style="border-radius: 15px;"></div>', unsafe_allow_html=True)
    else:
        st.error(f"⚠️ Kritik Hata: {gif_file} dosyası assets klasöründe bulunamadı!")
        st.image("https://img.icons8.com/fluency/300/robot-viewer.png", width=width)
...
col_app, col_ranking = st.columns([3.3, 1])
...
with col_app:
    if not st.session_state.is_logged_in:
        st.markdown('<div class="pito-bubble">Merhaba! Ben <b>Pito</b>. Python macerasına hazır mısın?</div>', unsafe_allow_html=True)
    ...
    if st.session_state.graduation_view:
        ...
        st.balloons(); show_pito_gif(400)
        st.success(f"Nusaybin'in gururu oldun! Puanın: {st.session_state.total_score}")
        c1, c2 = st.columns(2)
        with c1:
    ...
    # --- 7. EKSİKSİZ VE DERİN MÜFREDAT (40 ADIM) ---
    training_data = [
        {"module_title": "1. İletişim: print() ve Metin Dünyası", "exercises": [
            {"msg": "**Pito'nun Notu:** Python'ın dünyayla konuşma yolu `print()` fonksiyonudur. Ekrana yazacağın metinleri mutlaka tırnak (' ') içine almalısın. Tırnaklar bilgisayara 'bu bir yazıdır' mesajını verir.\n\n**GÖREV:** Editor içine tam olarak **'Merhaba Pito'** yaz!", "task": "print('___')", "check": lambda c, o, i: "Merhaba Pito" in o, "solution": "print('Merhaba Pito')", "hint": "Metnin başına ve sonuna tırnak koy."},
            {"msg": "**Pito'un Notu:** Sayılar (Integer), metinlerden farklıdır; tırnak gerektirmezler. Tırnak koyarsan Python onu sayı değil, yazı olarak görür ve üzerinde matematik yapamaz.\n\n**GÖREV:** Boşluğa tırnak kullanmadan sadece **100** yaz.", "task": "print(___)", "check": lambda c, o, i: "100" in o, "solution": "print(100)", "hint": "Rakamları doğrudan yaz."},
            {"msg": "**Pito'nun Notu:** Virgül (`,`) farklı veri tiplerini (metin ve sayı gibi) aynı satırda birleştirirken otomatik boşluk koyar.\n\n**GÖREV:** 'Puan:' metni ile **100** sayısını birleştirmek için boşluğa **100** yaz.", "task": "print('Puan:', ___)", "check": lambda c, o, i: "100" in o, "solution": "print('Puan:', 100)", "hint": "Virgülden sonra 100 yaz."},
            {"msg": "**Pito'nun Notu:** `#` işareti Python'a 'Bu satırı okuma' demektir. Buna 'Yorum Satırı' diyoruz. Sadece geliştiricilerin not alması içindir.\n\n**GÖREV:** Satırın en başına **#** işaretini koy.", "task": "___ bu bir nottur", "check": lambda c, o, i: "#" in c, "solution": "# bu bir nottur", "hint": "Diyez (#) işaretini kullan."},
            {"msg": "**Pito'nun Notu:** `\\n` metni alt satıra böler. Sanki klavyede Enter'a basılmış gibi davranır.\n\n**GÖREV:** Boşluğa **\\n** yazarak kelimeleri alt alta getir.", "task": "print('Üst' + '___' + 'Alt')", "check": lambda c, o, i: "Üst\nAlt" in o, "solution": "print('Üst\\nAlt')", "hint": "\\n kaçış karakteridir."}
        ]},
        {"module_title": "2. Hafıza: Değişkenler ve input()", "exercises": [
            {"msg": "**Pito'nun Notu:** Değişkenler bellekteki kutulardır. `=` işareti sağdaki değeri soldaki kutunun içine koyar (Atama).\n\n**GÖREV:** `yas` kutusuna sayısal olarak **15** ata.", "task": "yas = ___", "check": lambda c, o, i: "15" in str(i.get('yas', '')), "solution": "yas = 15", "hint": "Sadece 15 yaz."},
            {"msg": "**Pito'nun Notu:** `input()` programı durdurur ve kullanıcıdan bir bilgi bekler. Bu bilgiyi her zaman metin (String) olarak saklar.\n\n**GÖREV:** Kullanıcıdan adını almak için boşluğa **input** yaz.", "task": "ad = ___('Adın: ')", "check": lambda c, o, i: "input" in c, "solution": "ad = input('Adın: ')", "hint": "Veri girişi fonksiyonunu kullan."},
            {"msg": "**Pito'nun Notu:** Sayıları metne çevirmemiz gerektiğinde (Casting) `str()` fonksiyonunu kullanırız.\n\n**GÖREV:** 10 sayısını metne çeviren **str** komutunu yerleştir.", "task": "print(___(10))", "check": lambda c, o, i: "str" in c, "solution": "print(str(10))", "hint": "str(sayı) yapısı."},
            {"msg": "**Pito'nun Notu:** Matematik yapabilmek için `input()` ile gelen veriyi `int()` ile tam sayıya çevirmelisin.\n\n**GÖREV:** Dışa **int**, içe **input** yazarak sayı girişi al.", "task": "n = ___(___('S: '))", "check": lambda c, o, i: "int" in c and "input" in c, "solution": "n = int(input('S: '))", "hint": "int(input()) yapısı."},
            {"msg": "**Pito'nun Notu:** `isim` kutusuna tırnak kullanarak Pito ismini ata.\n\n**GÖREV:** Boşluğa tırnak içinde **'Pito'** yaz.", "task": "isim = ___", "check": lambda c, o, i: "Pito" in str(i.get('isim', '')), "solution": "isim = 'Pito'", "hint": "Tırnak kullanmayı unutma."}
        ]},
        {"module_title": "3. Mantık: Karar Yapıları (If-Else)", "exercises": [
            {"msg": "**Pito:** Programların beyni `if` bloğudur. Eşitlik sorgusunda mutlaka `==` kullanmalısın.\n\n**GÖREV:** Sayı 10'a eşitse kontrolü için **==** koy.", "task": "if 10 ___ 10: print('OK')", "check": lambda c, o, i: "==" in c, "solution": "if 10 == 10:\n    print('OK')", "hint": "== operatörü."},
            {"msg": "**Pito:** `else:` şart sağlanmadığında çalışan 'B Planı'dır.\n\n**GÖREV:** Boşluğa **else** yaz.", "task": "if 5 > 10: pass\n___: print('Hata')", "check": lambda c, o, i: "else" in c, "solution": "if 5 > 10: pass\nelse:\n    print('Hata')", "hint": "else: yapısı."},
            {"msg": "**Pito:** Birden fazla şartı denetlemek için `elif` kullanılır.\n\n**GÖREV:** Boşluğa **elif** yaz.", "task": "p = 60\nif p < 50: pass\n___ p > 50: print('G')", "check": lambda c, o, i: "elif" in c, "solution": "if p < 50: pass\nelif p > 50:\n    print('G')", "hint": "elif kullan."},
            {"msg": "**Pito:** `and` bağlacı iki tarafın da doğru olmasını bekler.\n\n**GÖREV:** Boşluğa **and** yaz.", "task": "if 1==1 ___ 2==2: print('OK')", "check": lambda c, o, i: "and" in c, "solution": "if 1==1 and 2==2:\n    print('OK')", "hint": "and anahtarı."},
            {"msg": "**Pito:** `!=` operatörü 'eşit değilse' anlamına gelir.\n\n**GÖREV:** Boşluğa **!=** koy.", "task": "s = 5\nif s ___ 0: print('Var')", "check": lambda c, o, i: "!=" in c, "solution": "if s != 0:\n    print('Var')", "hint": "!= operatörü."}
        ]},
        {"module_title": "4. Otomasyon: For ve While Döngüleri", "exercises": [
            {"msg": "**range:** `range(5)` komutu 0'dan 4'e kadar sayı üretir.\n\n**GÖREV:** Boşluğa **range** yaz.", "task": "for i in ___(5): print(i)", "check": lambda c, o, i: "range" in c, "solution": "for i in range(5):\n    print(i)", "hint": "range fonksiyonu."},
        ...
        {"module_title": "6. Modülerlik: Fonksiyonlar ve Sözlükler", "exercises": [
            ...
            {"msg": "**Sözlük:** `{anahtar: değer}` çiftleridir.\n\n**GÖREV:** Boşluğa tırnak içinde **'Pito'** yaz.", "task": "d = {'ad': ___}", "check": lambda c, o, i: "Pito" in str(i.get('d', {})), "solution": "d = {'ad': 'Pito'}", "hint": "Sözlük değeri."},
    ...
    c_box_i, c_box_m = st.columns([1.4, 3.6])
    with c_box_i: show_pito_gif(320)
    with c_box_m:
    ...
    if not is_review_mode and st.session_state.fail_count < 4 and not st.session_state.exercise_passed:
        ...
        if "input" in curr_ex['solution']:
            ...
            custom_input = st.text_input("📝 Değer giriniz:", key=f"inp_{st.session_state.current_module}_{st.session_state.current_exercise}").strip()
            if st.session_state.no_input_error: st.warning("⚠️ Önce kutuya veri gir!")

        code = st_ace(value=curr_ex['task'], language="python", theme="dracula", font_size=16, height=200, key=f"ace_{st.session_state.current_module}_{st.session_state.current_exercise}", auto_update=True)
        
        if st.button("🔍 Kodumu Kontrol Et", use_container_width=True):
            if "___" in code: st.session_state.feedback_msg = "⚠️ Lütfen boşluğu doldur!"; st.rerun()
                ...
                try:
                    ...
                    if st.session_state.fail_count < 4:
                        st.session_state.feedback_msg = f"❌ {st.session_state.fail_count}. Hata! -5 Puan. Tekrar dene!"
                    else:
                        st.session_state.feedback_msg = "🌿 4 kez hata yaptın. Aşağıdaki çözümü incele ve sonraki soruya geç."
                st.rerun()
