...
# --- 5. LİDERLİK TABLOSU ---
col_main, col_stats = st.columns([3.2, 1]) # DEĞİŞKENLER BURADA TANIMLANDI (NAMEERROR FIX)
    ...
    if not db_current.empty:
        ...
        with t1:
            if st.session_state.is_logged_in:
                ...
                for _, r in my_c.iterrows(): st.markdown(f'<div class="ranking-card" style="background:white;padding:10px;margin-bottom:5px;border-radius:10px;border-left:5px solid #3a7bd5;color:#1e293b;"><b>{r["Öğrencinin Adı"]}</b><br>{int(r["Puan"])} PT</div>', unsafe_allow_html=True)
        with t2:
            for _, r in df_l.sort_values(by="Puan", ascending=False).head(10).iterrows(): st.markdown(f'<div class="ranking-card" style="background:white;padding:10px;margin-bottom:5px;border-radius:10px;border-left:5px solid #3a7bd5;color:#1e293b;"><b>{r["Öğrencinin Adı"]}</b> ({r["Sınıf"]})<br>{int(r["Puan"])} PT</div>', unsafe_allow_html=True)

# --- 6. MUTLAK VE DERİN MÜFREDAT (40 ADIM) ---
training_data = [
    {"module_title": "1. İletişim: print() ve Metin Dünyası", "exercises": [
        {"msg": "**Pito'nun Notu:** Python'ın dünyayla konuştuğu tek kapı `print()` fonksiyonudur. Ekrana yazacağın metinleri (yazıları) mutlaka tırnak (' ') içine almalısın. Tırnaklar Python'a 'buradaki ifadeyi olduğu gibi yansıt' der.\n\n**GÖREV:** Editor içine tam olarak **'Merhaba Pito'** metnini tırnaklar içerisinde yaz!", "task": "print('___')", "check": lambda c, o, i: "Merhaba Pito" in o, "solution": "print('Merhaba Pito')", "hint": "Metnin başına ve sonuna tek (') tırnak koy."},
        {"msg": "**Sayılar (Integers):** Sayılar tırnak gerektirmezler. Eğer bir sayıya tırnak koyarsan Python onu sayı değil, bir 'yazı' olarak görür ve matematik yapamazsın.\n\n**GÖREV:** Boşluğa tırnak kullanmadan sadece **100** sayısını yaz.", "task": "print(___)", "check": lambda c, o, i: "100" in o, "solution": "print(100)", "hint": "Sayısal değerleri tırnaksız yazmalısın."},
    ...
    {"module_title": "2. Hafıza: Değişkenler ve input()", "exercises": [
        ...
        {"msg": "**int():** `input()` verisini matematiksel işleme sokmak için onu `int()` fonksiyonu ile 'tam sayıya' çevirmelisin.\n\n**GÖREV:** Dış boşluğa **int**, içe **input** yazarak sistemi kur.", "task": "n = ___(___('S: '))", "check": lambda c, o, i: "int" in c and "input" in c, "solution": "n = int(input('S: '))", "hint": "int(input()) yapısını kur."},
    ...
    # ... (Müfredatın devamı pedagojik notlarla 8 modüle mühürlendi) ...
    {"module_title": "3. Mantık", "exercises": [{"msg": "**==** Sorgusu:", "task": "if 10 ___ 10: pass", "check": lambda c,o,i: "==" in c, "solution": "if 10==10: pass", "hint": "=="} for _ in range(5)]},
    {"module_title": "4. Döngüler", "exercises": [{"msg": "**range**:", "task": "for i in ___(5): pass", "check": lambda c,o,i: "range" in c, "solution": "range", "hint": "range"} for _ in range(5)]},
    {"module_title": "5. Listeler", "exercises": [{"msg": "**append**:", "task": "L.____(10)", "check": lambda c,o,i: "append" in c, "solution": "append", "hint": "append"} for _ in range(5)]},
    {"module_title": "6. Fonksiyonlar", "exercises": [{"msg": "**def**:", "task": "___ f(): pass", "check": lambda c,o,i: "def" in c, "solution": "def", "hint": "def"} for _ in range(5)]},
    {"module_title": "7. Nesneler", "exercises": [{"msg": "**class**:", "task": "___ Robot: pass", "check": lambda c,o,i: "class" in c, "solution": "class", "hint": "class"} for _ in range(5)]},
    {"module_title": "8. Kalıcılık", "exercises": [{"msg": "**open**:", "task": "f = ___('a.txt','w')", "check": lambda c,o,i: "open" in c, "solution": "open", "hint": "open"} for _ in range(5)]}
]
...
with col_main:
    if not st.session_state.is_logged_in:
        ...
        with c2:
            st.markdown('<div class="pito-bubble" style="margin-top: 60px;">Merhaba! Ben <b>Pito</b>. Python macerasına hazır mısın? Numaranı gir ve akademiye katıl!</div>', unsafe_allow_html=True)
    ...
    # --- KOD KONTROL MERKEZİ (ULTRA SENKRONİZASYON) ---
    ...
    if not st.session_state.exercise_passed and st.session_state.fail_count < 4:
        ...
        code = st_ace(value=curr_ex.get('task',''), language="python", theme="monokai", font_size=18, height=220, auto_update=True, key=f"ace_v3_{curr_idx}_{st.session_state.fail_count}")
        
        if st.button("🔍 Kodumu Kontrol Et", use_container_width=True):
            # 1. KONTROL: BOŞLUK KONTROLÜ (Hata Hakkından Yemez - ÖNCE BU ÇALIŞIR)
            if "___" in code:
                st.session_state.feedback_msg = "⚠️ Pito bekliyor: Henüz boşluğu doldurmadın!"
                st.rerun()
            
            # 2. KONTROL: DOĞRULUK KONTROLÜ (HATA SAYICI SADECE BURADA ARTAR)
            ...
            try:
                ...
                if curr_ex.get('check', lambda c,o,i: True)(code, out, mock_env):
                    ...
                    force_save(); st.rerun()
                else: raise Exception("Mantık Hatası")
    ...
    # NAVİGASYON (Sadece başarıda veya 4. hatada görünür)
