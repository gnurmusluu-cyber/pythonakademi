...
training_data = [
    {"module_title": "1. Giriş ve Çıktı", "exercises": [
        ...
        {"msg": "**'Puan:'** metni ile **100** sayısını yan yana yazdır.", "task": "print('Puan:', ___)", "check": lambda c, o: "100" in o, "solution": "print('Puan:', 100)", "has_output": True},
    ...
    {"module_title": "3. Karar Yapıları", "exercises": [
        ...
        {"msg": "Büyük veya eşittir için **>=** kullan.", "task": "if 5 ___ 5: print('Z')", "check": lambda c, o: ">=" in c, "solution": "if 5 >= 5: print('Z')", "has_output": True},
    ...
    {"module_title": "6. Fonksiyonlar ve Veriler", "exercises": [
        {"msg": "**def** ile fonksiyon tanımla.", "task": "___ f(): print('X')", "check": lambda c, o: "def" in c, "solution": "def f(): print('X')", "has_output": False},
...
with col_main:
    ...
    if st.session_state.db_module >= 8:
        ...
        with c1:
            if st.button("🔄 Eğitimi Tekrar Al (Sıfırla)"):
                ...
                force_save(); st.rerun()
        with c2:
            if st.button("🏆 Liderlik Listesinde Kal"): st.info("Başarın listede kalmaya devam edecek!")
        st.divider()
    ...
    # FAIL_COUNT >= 3 İSE ÇÖZÜMÜ GÖSTER
    if st.session_state.fail_count >= 3 and not is_locked:
        ...
        st.warning("⚠️ Çözümü gördün! Şimdi son bir kez kendi kodunu yazıp 'Kontrol Et'e basmalısın.")
        ...
        if st.button("🔍 Kontrol Et", use_container_width=True):
            ...
            if out.startswith("⚠️"): st.error(out)
                ...
                if st.session_state.fail_count >= 4:
                    st.error("❌ Son hakkın da bitti. Bu egzersizden puan kazanamadın.")
                    st.session_state.exercise_passed = True
                else:
                    st.error(f"Hatalı! {st.session_state.fail_count}. deneme başarısız. " + ("Çözüm açıldı!" if st.session_state.fail_count==3 else ""))
            else:
