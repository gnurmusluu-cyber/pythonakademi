...
if 'is_logged_in' not in st.session_state:
    st.session_state.update({
        ...
        'db_module': 0, 'db_exercise': 0, 'is_logged_in': False,
...
# --- MODERN UI CSS (APPLY BUTONU VE TÜM GÖLGELERİ SİL) ---
st.markdown("""
    <style>
    header {visibility: hidden;}
    .main .block-container {padding-top: 1rem; background-color: #0f172a;}
    
    /* ACE EDITOR: APPLY VE TÜM İÇ BUTONLARI CSS İLE KESİN OLARAK YOK ET */
    iframe { border-radius: 15px !important; border: 2px solid #3a7bd5 !important; }
    [data-testid="stAceApplyButton"], .ace-apply-button, .ace_button, button[class*="ace"] { 
...
training_data = [
    ...
    {"module_title": "5. Gruplama", "exercises": [{"msg": "**append**:", "task": "L.____(10)", "check": lambda c,o,i: "append" in c, "solution": "append", "hint": "append"} for _ in range(5)]},
    ...
    {"module_title": "7. OOP Dünyası", "exercises": [{"msg": "**class**:", "task": "___ Robot: pass", "check": lambda c,o,i: "class" in c, "solution": "class", "hint": "class"} for _ in range(5)]},
...
with col_main:
    ...
    if sum(st.session_state.completed_modules) >= 8:
        ...
        st.markdown('<div class="pito-bubble" style="text-align:center;">🎊 <b>MEZUNİYET MÜHÜRLENDİ!</b><br>Nusaybin laboratuvarının gururusun Python Kahramanı!</div>', unsafe_allow_html=True); st.balloons()
    ...
    # --- KOD KONTROL MERKEZİ (ATOMİK MANTIK) ---
    ...
    if not st.session_state.exercise_passed and st.session_state.fail_count < 4:
        # FORM MANTIKLI RADİKAL KONTROL: Veriyi butona basıldığı an paketler
        with st.form(key=f"pito_lock_{curr_idx}"):
            ...
            if submit:
                # 1. KONTROL: Henüz doldurulmamışsa HATA SAYMA!
                if "___" in code:
                    st.session_state.feedback_msg = "⚠️ Pito bekliyor: Boşluğu doldurmalısın!"; st.rerun()
                
                # 2. KONTROL: KOD DOĞRULAMA (SUCCESS BARAJI)
                ...
                try:
                    ...
                    if curr_ex.get('check', lambda c,o,i: True)(code, out, mock_env):
                        # KESİN BAŞARI: Hata sayacını bypass et
                        ...
                        force_save(); st.rerun()
                    else: raise Exception("Wrong Answer") # Kod çalıştı ama sonuç yanlış
    ...
    # NAVİGASYON (Sadece tam başarıda veya 4. hatadan sonra)
