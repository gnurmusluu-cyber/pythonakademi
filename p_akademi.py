...
st.markdown("""
    ...
    .solution-guide { background-color: #fef2f2 !important; border: 3px solid #ef4444 !important; border-radius: 15px; padding: 20px; margin: 15px 0; color: #1e1e1e !important; }
    .hint-guide { background-color: #fffbeb !important; border: 3px solid #f59e0b !important; border-radius: 15px; padding: 20px; margin: 15px 0; color: #1e1e1e !important; }
    ...
    .stButton > button { width: 100%; border-radius: 15px; height: 4.2em; background: linear-gradient(45deg, #3a7bd5, #00d2ff) !important; color: white !important; font-weight: bold; border: none; font-size: 1.15rem; box-shadow: 0 4px 15px rgba(58, 123, 213, 0.3); }
    [data-testid="stTextInput"] { border: 2.8px solid #3a7bd5 !important; border-radius: 15px; font-size: 1.1rem; }
    </style>
    """, unsafe_allow_html=True)

# --- 3. KESİN ÇÖZÜM: MACBOOK/SAFARI UYUMLU BASE64 GIF SİSTEMİ ---
def get_pito_gif(gif_name):
    # Pathlib ile her türlü işletim sisteminde assets klasörüne erişim mühürlendi
...
def show_pito_gif(width=450):
    ...
    b64_data = get_pito_gif(gif_file)
    
    if b64_data:
        # Rastgele query param ekleyerek tarayıcı cache'ini bypass ediyoruz
        t = time.time()
        st.markdown(f'<div style="display: flex; justify-content: center; margin-bottom: 15px;"><img src="data:image/gif;base64,{b64_data}?t={t}" width="{width}" style="border-radius: 20px;"></div>', unsafe_allow_html=True)
    else:
...
with col_app:
    if not st.session_state.is_logged_in:
        st.markdown('<div class="pito-bubble">Merhaba Geleceğin Yazılımcısı! Ben <b>Pito</b>. Nusaybin laboratuvarında Python dünyasına adım atmaya hazır mısın?</div>', unsafe_allow_html=True)
    ...
    if st.session_state.graduation_view:
        ...
        st.markdown('<div class="pito-bubble">🎊 <b>TEBRİKLER Python Kahramanı!</b> Nusaybin laboratuvarının ilk mezunlarından biri oldun.</div>', unsafe_allow_html=True)
    ...
    training_data = [
        ...
        {"module_title": "2. Hafıza: Değişkenler ve input()", "exercises": [
            ...
            {"msg": "**Pito'nun Notu:** Metin (String) verilerini hafızaya alırken tırnak şarttır. Değişken isimlerinde rakamla başlamamaya ve boşluk kullanmamaya dikkat etmelisin.\n\n**GÖREV:** `isim` değişkenine **'Pito'** metnini ata.", "task": "isim = '___'", "check": lambda c, o, i: "Pito" in str(i.get('isim', '')), "solution": "isim = 'Pito'", "hint": "Metni tırnaklar içerisine tam olarak Pito şeklinde yaz."},
            {"msg": "**Pito'nun Notu:** `input()` programı durdurur ve kullanıcıdan bir bilgi bekler. Python bu bilgiyi ne olursa olsun her zaman 'metin' (String) olarak algılar.\n\n**GÖREV:** Kullanıcıdan adını almak için boşluğa veri alma fonksiyonu olan **input** yaz.", "task": "ad = ___('Adın: ')", "check": lambda c, o, i: "input" in c, "solution": "ad = input('Adın: ')", "hint": "input kelimesini kullan."},
    ...
    # GÖRSEL HİYERARŞİ: Pito (1.8) ve Konuşma Balonu (3.2) arasında altın denge kuruldu.
    c_box_i, c_box_m = st.columns([1.8, 3.2])
