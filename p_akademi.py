...
st.markdown("""
    ...
    [data-testid="stAceApplyButton"], .ace-apply-button { display: none !important; }
    iframe { border: 2.5px solid #3a7bd5 !important; border-radius: 15px !important; }
...
with col_main:
    ...
    # --- KOD KONTROL MERKEZİ (RADİKAL ÇÖZÜM) ---
    ...
    if not st.session_state.exercise_passed and st.session_state.fail_count < 4:
        # FORM MANTIĞI: Butona basana kadar veri backend'e gitmez, butona basıldığı an Ace'deki en güncel veri okunur.
        with st.form(key=f"pito_form_{curr_idx}"):
            # Ace Editor ayarları senkronizasyonu zorlar
            ...
            if submit:
                # 1. Aşama: Boşluk Kontrolü (Hata hakkından yemez)
                if "___" in code:
                    st.session_state.feedback_msg = "⚠️ Pito bekliyor: Boşluğu doldurmalısın!"
                    st.rerun()
                
                # 2. Aşama: Çalıştırma ve Doğrulama
                old_stdout, new_stdout = sys.stdout, StringIO(); sys.stdout = new_stdout
                try:
                    # Kontrol ortamı
                    mock_env = {"print": print, "input": lambda x: "10", "int": int, "str": str, "yas": 0, "isim": "", "L": [10,20]}
