...
# Liderlik Tablolarını Sidebar'da göster (Login öncesi/sonrası her zaman çalışır)
    ...
    with col_mid:
        ...
        if in_no:
            if not in_no.isdigit():
                st.session_state.login_error = "⚠️ Okul numarası sadece rakamlardan oluşmalı!"
                st.rerun()
            else:
                ...
                if not user_data.empty:
                    ...
                    with c2:
                        if st.button("❌ Hayır, Değilim"):
                            st.session_state.login_error = "🔴 Lütfen okul numaranı tekrar kontrol ederek gir!"
                            st.rerun()
                else:
...
training_data = [
    ...
    {"module_title": "2. Hafıza: Değişkenler ve Atama", "exercises": [
        ...
        {"msg": "**Konu:** Metin atarken tırnak şarttır.\n\n**Görev:** `isim` kutusuna **'Pito'** değerini ata.", "task": "isim = '___'", "check": lambda c, o, i: "Pito" in o, "solution": "isim = 'Pito'", "hint": "Boşluğa Pito yaz."},
        {"msg": "**Girdi:** `input()` kullanıcıdan bilgi bekler.\n\n**Görev:** Boşluğa **input** fonksiyonunu yaz.", "task": "ad = ___('Adın: ')", "check": lambda c, o, i: "input" in c, "solution": "ad = input('Adın: ')", "hint": "input kelimesini kullan."},
        {"msg": "**Casting:** `str()` sayıları metne çevirir.\n\n**Görev:** 10 sayısını metne çeviren **str** fonksiyonunu yaz.", "task": "print(___(10))", "check": lambda c, o, i: "str" in c, "solution": "print(str(10))", "hint": "str yazmalısın."},
        {"msg": "**Tam Sayı:** `int()` metni sayıya çevirir.\n\n**Görev:** Dış boşluğa **int**, içe **input** yaz.", "task": "n = ___(___('S: '))", "check": lambda c, o, i: "int" in c and "input" in c, "solution": "n = int(input('S: '))", "hint": "int(input()) yapısını kur."}
    ]},
    {"module_title": "3. Karar Yapıları: If-Else Dünyası", "exercises": [
        {"msg": "Eşitlik kontrolü için `==` kullanılır.\n\n**Görev:** Sayı 10'a eşitse 'OK' yazdıran operatörü (**==**) yaz.", "task": "if 10 ___ 10: print('OK')", "check": lambda c, o, i: "==" in c, "solution": "if 10 == 10:", "hint": "Çift eşittir kullan."},
        {"msg": "Şart yanlışsa `else:` çalışır.\n\n**Görev:** Boşluğa **else** yaz.", "task": "if 5 > 10: pass\n___: print('X')", "check": lambda c, o, i: "else" in c, "solution": "else:", "hint": "else: yaz."},
        {"msg": "`elif` çoklu şart demektir.\n\n**Görev:** Puan 50'den büyükse 'Geçti' yazacak **elif** şartını koy.", "task": "p = 60\nif p < 50: pass\n___ p > 50: print('Geçti')", "check": lambda c, o, i: "elif" in c, "solution": "elif p > 50:", "hint": "elif yaz."},
        {"msg": "`and` (ve) iki tarafın da doğru olmasını bekler.\n\n**Görev:** Boşluğa **and** yaz.", "task": "if 1==1 ___ 2==2: print('OK')", "check": lambda c, o, i: "and" in c, "solution": "and", "hint": "and yaz."},
        {"msg": "`!=` eşit değilse demektir.\n\n**Görev:** s sıfır değilse 'Var' yazdıran operatörü (**!=**) koy.", "task": "s = 5\nif s ___ 0: print('Var')", "check": lambda c, o, i: "!=" in c, "solution": "if s != 0:", "hint": "!= koy."}
    ]},
    {"module_title": "4. Otomasyon: For ve While Döngüleri", "exercises": [
        {"msg": "`for` döngüsü tekrar yapar. `range(5)` sayıları üretir.\n\n**Görev:** Boşluğa **range** yaz.", "task": "for i in ___(5): print(i)", "check": lambda c, o, i: "range" in c, "solution": "for i in range(5):", "hint": "range yaz."},
        {"msg": "`while` şart doğru oldukça döner.\n\n**Görev:** Boşluğa **while** yaz.", "task": "i = 0\n___ i == 0: i += 1", "check": lambda c, o, i: "while" in c, "solution": "while i == 0:", "hint": "while yaz."},
        {"msg": "`break` döngüyü bitirir.\n\n**Görev:** Boşluğa **break** yaz.", "task": "for i in range(5):\n if i==1: ___", "check": lambda c, o, i: "break" in c, "solution": "break", "hint": "break yaz."},
        {"msg": "`continue` o adımı atlar.\n\n**Görev:** Boşluğa **continue** yaz.", "task": "for i in range(3):\n if i==1: ___", "check": lambda c, o, i: "continue" in c, "solution": "continue", "hint": "continue yaz."},
        {"msg": "`in` anahtar kelimesi listede gezer.\n\n**Görev:** Listedeki harfleri basmak için **in** yaz.", "task": "for x ___ ['A', 'B']: print(x)", "check": lambda c, o, i: "in" in c, "solution": "for x in", "hint": "in yaz."}
    ]},
    {"module_title": "5. Gruplama: Listeler", "exercises": [
        {"msg": "Listeler `[]` içine yazılır.\n\n**Görev:** Boşluğa **10** yaz.", "task": "L = [___, 20]", "check": lambda c, o, i: "10" in c, "solution": "L = [10, 20]", "hint": "10 yaz."},
        {"msg": "İndeks 0'dan başlar.\n\n**Görev:** İlk elemana (50) erişmek için **0** yaz.", "task": "L = [50, 60]\nprint(L[___])", "check": lambda c, o, i: "50" in o, "solution": "L[0]", "hint": "0 yaz."},
        {"msg": "`.append()` sona ekler.\n\n**Görev:** Boşluğa **append** yaz.", "task": "L = [10]\nL.___ (30)", "check": lambda c, o, i: "append" in c, "solution": "L.append(30)", "hint": "append yaz."},
        {"msg": "`len()` boyut ölçer.\n\n**Görev:** Boşluğa **len** yaz.", "task": "L = [1, 2, 3]\nprint(___(L))", "check": lambda c, o, i: "3" in o, "solution": "len(L)", "hint": "len yaz."},
    ...
    {"module_title": "6. Modülerlik: Fonksiyonlar ve Sözlükler", "exercises": [
        {"msg": "`def` fonksiyon tanımlar.\n\n**Görev:** Boşluğa **def** yaz.", "task": "___ pito(): print('Hi')", "check": lambda c, o, i: "def" in c, "solution": "def pito():", "hint": "def yaz."},
        {"msg": "**Sözlükler**, `{key: value}` şeklinde tutar.\n\n**Görev:** 'ad' anahtarına **'Pito'** yaz.", "task": "d = {'ad': '___'}", "check": lambda c, o, i: "Pito" in o, "solution": "d = {'ad': 'Pito'}", "hint": "Pito yaz."},
        {"msg": "**Tuple**, listeye benzer ama `()` ile kurulur.\n\n**Görev:** Boşluğa **1** yaz.", "task": "t = (___, 2)", "check": lambda c, o, i: "1" in c, "solution": "t = (1, 2)", "hint": "1 yaz."},
        ...
        {"msg": "`return` sonucu döndürür.\n\n**Görev:** Boşluğa **return** yaz.", "task": "def f(): ___ 5", "check": lambda c, o, i: "return" in c, "solution": "return 5", "hint": "return yaz."}
    ]},
    {"module_title": "7. OOP: Nesne Dünyası", "exercises": [
        {"msg": "`class` bir fabrikadır.\n\n**Görev:** Robot kalıbı için **class** yaz.", "task": "___ Robot: pass", "check": lambda c, o, i: "class" in c, "solution": "class Robot:", "hint": "class yaz."},
        {"msg": "Kalıptan nesne üretmek için parantez `()` kullanılır.\n\n**Görev:** Boşluğa **Robot()** yaz.", "task": "class Robot: pass\nr = ___", "check": lambda c, o, i: "Robot()" in c, "solution": "r = Robot()", "hint": "Robot() yaz."},
        {"msg": "Özellikler nokta (`.`) ile atanır.\n\n**Görev:** r'nin **renk** özelliğine 'Mavi' yaz.", "task": "class R: pass\nr = R()\nr.___ = 'Mavi'", "check": lambda c, o, i: "renk" in c, "solution": "r.renk = 'Mavi'", "hint": "renk yaz."},
        {"msg": "`self` nesnenin kendisidir.\n\n**Görev:** Parantez içine **self** yaz.", "task": "class R:\n def s(___): print('X')", "check": lambda c, o, i: "self" in c, "solution": "def s(self):", "hint": "self yaz."},
        {"msg": "Metotlar nokta ile çağrılır.\n\n**Görev:** s() metodunu çalıştırmak için **s()** yaz.", "task": "class R:\n def s(self): pass\nr = R()\nr.___()", "check": lambda c, o, i: "s()" in c, "solution": "r.s()", "hint": "s() yaz."}
    ]},
    {"module_title": "8. Kalıcılık: Dosya Yönetimi", "exercises": [
        {"msg": "`open()` ile dosya açarız. **'w'** yazma modudur.\n\n**Görev:** Boşlukları **open** ve **'w'** ile doldur.", "task": "f = ___('n.txt', '___')", "check": lambda c, o, i: "open" in c and "w" in c, "solution": "open('n.txt', 'w')", "hint": "open ve w kullan."},
        {"msg": "`.write()` mühürler.\n\n**Görev:** Boşluğa **write** yaz.", "task": "f = open('t.txt', 'w')\nf.___('X')", "check": lambda c, o, i: "write" in c, "solution": "f.write('X')", "hint": "write yaz."},
        {"msg": "Okuma modu **'r'** (read) modudur.\n\n**Görev:** Boşluğa **r** harfini koy.", "task": "f = open('t.txt', '___')", "check": lambda c, o, i: "r" in c, "solution": "f = open('t.txt', 'r')", "hint": "r yaz."},
...
# KOD PANELİ
if is_review_mode:
    st.markdown('<div class="solution-guide"><div class="solution-header">📖 Arşiv Kaydı</div></div>', unsafe_allow_html=True); st.code(curr_ex['solution'], language="python")
else:
        ...
        if st.button("🔍 Kodumu Kontrol Et"):
            if "___" in code: st.session_state.feedback_msg = "⚠️ Pito bekliyor: Lütfen boşluğu doldur!"; st.rerun()
                ...
                try:
                    ...
                    msgs = ["🌟 Harika bir başlangıç! Küçük bir pürüz çıktı.", "💪 Pes etmek yok! Hatalar en iyi öğretmenlerdir.", "🚀 Yolun sonuna yaklaştın ama vazgeçme! İpucuna bak."]
