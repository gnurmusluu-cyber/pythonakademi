...
training_data = [
    ...
    # Diğer modüller baseline yapısıyla devam eder...
]
...
with col_main:
    ...
    if st.session_state.db_module >= 8:
        ...
        st.success("🎉 Tebrikler! Eğitimi Bitirdin.")
        c1, c2 = st.columns(2)
        with c1:
    ...
    if new_m_idx != st.session_state.current_module:
        st.session_state.update({'current_module': new_m_idx, 'current_exercise': 0, 'fail_count': 0, 'exercise_passed': False, 'feedback_msg': ""}); st.rerun()
    ...
    if is_locked:
        ...
        if st.session_state.fail_count < 4:
                ...
                if "___" in code:
                    ...
                    if out.startswith("❌") or not curr_ex['check'](code, out):
                        ...
                        if st.session_state.fail_count < 4:
                            ...
                            st.session_state.feedback_msg = "" # Temizle
                        st.rerun()
                    else:
            ...
            # --- KRİTİK: DÖNÜT MESAJI (TAM İSTEDİĞİNİZ YERDE) ---
        ...
        # 4. HATA YAPILDIYSA FINAL MESAJI
        if st.session_state.fail_count >= 4:
