...
training_data = [
    {"module_title": "1. Giriş ve Çıktı", "exercises": [
        ...
        {"msg": "Python'da sayılar metinlerden farklıdır. Matematiksel bir değer olan sayıları ekrana yazdırırken **tırnak işareti kullanmayız.** Bu sayede Python onların 'sayı' olduğunu anlar. Ekrana 100 sayısını yazdır.", "task": "print(___)", "check": lambda c, o: "100" in o, "solution": "print(100)"},
        ...
        {"msg": "**# (Diyez)** işaretiyle başlayan satırlar Python tarafından okunmaz ve çalıştırılmaz. Buna 'Yorum Satırı' denir. Bir yorum satırı ekle.", "task": "___ Bu bir yorumdur", "check": lambda c, o: "#" in c, "solution": "# Bu bir yorumdur"},
        {"msg": "Metinlerin içinde bir alt satıra geçmek için özel karakter: **'\\n'**. Üst ve Alt kelimelerini tek print içinde farklı satırlarda yazdır.", "task": "print('Üst' + '___' + 'Alt')", "check": lambda c, o: "\n" in o, "solution": "print('Üst' + '\\n' + 'Alt')"}
    ]},
    ...
    {"module_title": "8. Dosya İşlemleri", "exercises": [
        {"msg": "open() fonksiyonu kapıyı açar. 'w' (write) yazılabilir demektir. n.txt dosyasını 'w' moduyla aç.", "task": "dosya = ___('n.txt', '___')", "check": lambda c, o: "open" in c and "'w'" in c, "solution": "dosya = open('n.txt', 'w')\nprint('Dosya yazma modunda açıldı.')"},
        {"msg": "write() metodu içeriğe yazar. 'Pito' yazdır ve dosyayı kapat.", "task": "f = open('t.txt', 'w'); f.___('Pito'); f.close()", "check": lambda c, o: "write" in c, "solution": "f = open('t.txt', 'w'); f.write('Pito'); f.close()"},
        {"msg": "'r' (read) yalnızca okunabilir kipiyle t.txt dosyasını aç.", "task": "f = open('t.txt', '___')", "check": lambda c, o: "'r'" in c, "solution": "f = open('t.txt', 'r'); f.close()"},
        {"msg": "read() içeriği bütünüyle getirir. Dosyayı oku ve print ile bas.", "task": "f = open('t.txt', 'r'); print(f.___()); f.close()", "check": lambda c, o: "read" in c, "solution": "f = open('t.txt', 'w'); f.write('Pito Akademi'); f.close(); f = open('t.txt', 'r'); print(f.read()); f.close()"},
...
with col_main:
    ...
    # İNDEX TABANLI SEÇİCİ (API HATASINI ÖNLER)
    ...
    if "input(" in code and not is_locked:
        u_in = st.text_input("👇 Pito Terminali: Değer yaz ve Kontrol Et'e bas:")
    ...
    # BUTON AKIŞ KONTROLÜ (İstenen Düzenleme)
    c_back, c_next = st.columns(2)
    with c_back:
        # Sadece İnceleme Modu'ndaysa "Önceki Adım" butonu görünür
