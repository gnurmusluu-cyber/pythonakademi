...
# --- MODERN UI CSS (APPLY BUTONUNU KÖKTEN KAZI) ---
st.markdown("""
    <style>
    header {visibility: hidden;}
    .main .block-container {padding-top: 1rem; background-color: #0f172a;}
    
    /* ACE EDITOR: APPLY BUTONUNU VE TÜM GÖLGELERİNİ SİL */
    iframe { border-radius: 15px !important; border: 2.5px solid #3a7bd5 !important; }
    div[class*="stAce"] button, .ace-apply-button, [data-testid="stAceApplyButton"] { 
    ...
    .pito-bubble { position: relative; background: #ffffff !important; border: 3.5px solid #3a7bd5 !important; border-radius: 30px !important; padding: 30px !important; color: #1e293b !important; font-weight: 500 !important; font-size: 1.2rem !important; line-height: 1.8 !important; }
...
# --- 4. MUTLAK MÜFREDAT (40 DERİN ADIM) ---
training_data = [
    ...
    # ... (3-8 arası modüller aynı derin pedagojik yapıyla devam eder) ...
...
with col_main:
    ...
    if sum(st.session_state.completed_modules) >= 8:
        ...
        st.markdown('<div class="pito-bubble" style="text-align:center;">🎊 <b>MEZUNİYET MÜHÜRLENDİ!</b></div>', unsafe_allow_html=True); st.balloons()
    ...
    # --- KOD KONTROL MERKEZİ (KESİN ÇÖZÜM: FORM KİLİDİ) ---
    ...
    if not st.session_state.exercise_passed and st.session_state.fail_count < 4:
        # FORM MANTIĞI: Butona basana kadar Ace Editor verisi Streamlit'e gönderilmez. Buton basıldığında 'PAKET' olarak gelir.
        with st.form(key=f"lock_{curr_idx}"):
            ...
            if submit:
                # ADIM 1: Placeholder/Boşluk Kontrolü (HATA SAYMA!)
                if "___" in code:
                    st.session_state.feedback_msg = "⚠️ Pito bekliyor: Henüz boşluğu doldurmadın!"; st.rerun()
                
                # ADIM 2: Kodun Çalıştırılması ve Doğrulanması
                ...
                try:
                    ...
                    # BAŞARI KOŞULU (Eğer doğruysa, fail_count asla artamaz)
                    ...
                    # HATA KOŞULU (HATA SAYICI SADECE BURADA ARTAR)
    ...
    # NAVİGASYON (Sadece tam başarıda veya 4. hatadan sonra görünür)
