...
def auto_save_progress():
    """Mükerrer kaydı önler ve anlık konumu buluta kilitler."""
    try:
        no = str(st.session_state.student_no).strip()
        score = int(st.session_state.total_score)
        
        # Sadece ileri gitmeyi sağlayan koordinat kilidi
        df_old = get_db()
        ...
        if not user_row.empty:
            ...
            # Eğer öğrenci geçmişe bakmıyorsa veri tabanı koordinatını güncelle
...
# --- 4. GİRİŞ EKRANI (FULL RECOVERY) ---
if st.session_state.student_name == "":
    st.markdown("<br>", unsafe_allow_html=True)
    _, col_mid, _ = st.columns([1, 2, 1])
    with col_mid:
        st.markdown('<div class="pito-bubble">Merhaba ben <b>Pito</b>! Birlikte Python\'ın eğlenceli dünyasına dalalım.</div>', unsafe_allow_html=True)
        ...
        if st.button("Atölyeye Gir 🚀"):
            if in_no.strip() and in_name.strip():
                ...
                if not user_data.empty:
                    ...
                    # --- KRİTİK: Widget index'ini girişte kilitliyoruz ---
                    st.session_state["mod_secici"] = st.session_state.current_module
                st.rerun()
...
training_data = [
    ...
    {"module_title": "2. Değişkenler ve Giriş", "exercises": [
        ...
        {"msg": "Kullanıcıdan veri al (input).", "task": "ad = ___('Ad: ')\nprint(ad)", "check": lambda c, o: "input" in c},
...
with col_main:
    ...
    # MODÜL SEÇİCİ (ZORLANMIŞ INDEX)
    mod_list = [f"{'✅' if st.session_state.completed_modules[i] else '📖'} {m['module_title']}" for i, m in enumerate(training_data)]
    # key="mod_secici" kullanarak session_state üzerinden kontrol sağlıyoruz
    sel_mod = st.selectbox("Gitmek istediğin Modül:", mod_list, key="mod_secici")
    m_idx = mod_list.index(sel_mod)
    
    # Eğer selectbox üzerinden manuel değişim yapılırsa
    ...
    # Kilit Kontrolü (Bitirilenler readonly)
    ...
    with c_img: st.image(PITO_IMG if os.path.exists(PITO_IMG) else "https://img.icons8.com/fluency/200/robot-viewer.png", width=180)
    with c_msg:
        st.info(f"##### 🗣️ Pito Diyor Ki:\n\n{curr_ex['msg']}")
        st.caption(f"Adım: {e_idx + 1}/5 {'🔒 (İnceleme Modu)' if is_locked else f'🎁 {st.session_state.current_potential_score} Puan'}")
    ...
    if not is_locked:
        if st.button("🔍 Görevi Kontrol Et", use_container_width=True):
            ...
            try:
                ...
                st.code(out if out else "Pito: Kod başarıyla çalıştı!")
                if curr_ex['check'](code, out) and "___" not in code:
                    ...
                    if ex_key not in st.session_state.scored_exercises:
                        ...
                        auto_save_progress() # PUANI ANLIK KAYDET
        ...
        if e_idx < 4:
            ...
            if st.button("🏆 Modülü Bitir"):
                if not is_locked:
                    ...
                    # Modül bittiğinde selectbox'ı da otomatik kaydır
                    st.session_state["mod_secici"] = st.session_state.db_module
                st.session_state.current_module = st.session_state.db_module
                st.session_state.current_exercise = 0
                auto_save_progress(); st.balloons(); st.rerun()

# --- 7. SAĞ PANEL: EĞLENCELİ LİDERLİK TABLOSU ---
with col_side:
