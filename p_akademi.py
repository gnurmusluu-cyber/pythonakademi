...
st.markdown("""
    ...
    /* ACE EDITOR: APPLY BUTONUNU CSS İLE TAMAMEN YOK ET */
...
training_data = [
    ...
    {"module_title": "2. Hafıza: Değişkenler ve input()", "exercises": [
        ...
        {"msg": "**İsimlendirme:** Değişken isimlerinde rakamla başlamamaya ve boşluk kullanmamaya dikkat et!\n\n**GÖREV:** `isim` değişkenine **'Pito'** metnini ata.", "task": "isim = '___'", "check": lambda c, o, i: i.get('isim') == 'Pito', "solution": "isim = 'Pito'", "hint": "Metni tırnaklar içine Pito olarak yaz."}
    ]},
...
with col_main:
    ...
    if sum(st.session_state.completed_modules) >= 8:
        ...
        st.markdown('<div class="pito-bubble" style="text-align:center;">🎊 <b>TEBRİKLER Python Kahramanı!</b><br>Süleyman Bölünmez Anadolu Lisesi seninle gurur duyuyor!</div>', unsafe_allow_html=True); st.balloons()
    ...
    # --- KOD KONTROL MERKEZİ (KESİN SENKRONİZASYON) ---
    ...
    if not st.session_state.exercise_passed and st.session_state.fail_count < 4:
        # Editör key'ine fail_count EKLEMİYORUZ (Metin silinmesin diye), sadece egzersiz id bağlıyoruz
        code = st_ace(value=curr_ex.get('task',''), language="python", theme="monokai", font_size=18, height=220, auto_update=True, key=f"ace_v2_{curr_idx}")
        
        if st.button("🔍 Kodumu Kontrol Et", use_container_width=True):
            # 1. KONTROL: Ace Editor'deki metin hala başlangıçtaki boşlukları içeriyor mu?
            # Boşlukları ve satır sonlarını temizleyerek karşılaştırıyoruz
            if "___" in code:
                st.session_state.feedback_msg = "⚠️ Pito bekliyor: Henüz boşluğu doldurmadın!"
                st.rerun()
            
            # 2. KONTROL: KODUN ÇALIŞTIRILMASI VE DOĞRULANMASI
            ...
            try:
                ...
                if curr_ex.get('check', lambda c,o,i: True)(code, out, mock_env):
                    ...
                    raise Exception("Mantık Hatası") # Kod çalıştı ama sonuç yanlış
    ...
    # NAVİGASYON (Sadece başarıda veya 4. hatadan sonra görünür)
