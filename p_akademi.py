...
st.markdown("""
    ...
    /* ACE EDITOR: APPLY BUTONUNU TAMAMEN SİL */
    [data-testid="stAceApplyButton"], .ace-apply-button { display: none !important; visibility: hidden !important; height: 0 !important; }
...
def show_pito_gif(width=450):
    ...
    b64 = load_gif_data(emotion_map.get(st.session_state.pito_emotion, "pito_dusunuyor.gif"))
    if b64:
...
with col_stats:
    ...
    if not db_current.empty:
        ...
        st.markdown(f'''<div style="background: linear-gradient(135deg, #FFD700, #F59E0B); color: black; border-radius: 15px; padding: 20px; text-align: center; font-weight: 900; margin-bottom: 25px; box-shadow: 0 10px 20px rgba(245,158,11,0.4);">🥇 LİDER SINIF: {top_c}</div>''', unsafe_allow_html=True)
...
training_data = [
    ...
    {"module_title": "2. Hafıza: Değişkenler ve input()", "exercises": [
        ...
        {"msg": "**int():** `input()` verisiyle matematik yapmak için onu `int()` fonksiyonu ile 'tam sayıya' çevirmelisin.\n\n**GÖREV:** Dış boşluğa **int**, içe **input** yazarak sayı girişi al.", "task": "n = ___(___('S: '))", "check": lambda c, o, i: "int" in c and "input" in c, "solution": "n = int(input('S: '))", "hint": "int(input()) yapısını kur."},
        {"msg": "**İsimlendirme:** Değişken isimlerinde rakamla başlamamaya ve boşluk kullanmamaya dikkat et! Python büyük-küçük harfe duyarlıdır.", "task": "isim = '___'", "check": lambda c, o, i: i.get('isim') == 'Pito', "solution": "isim = 'Pito'", "hint": "Tırnaklar içine Pito yaz."}
    ]},
    {"module_title": "3. Mantık: Karar Yapıları (If-Else)", "exercises": [
        ...
        {"msg": "**B Planı:** `else:` şart sağlanmadığında devreye giren yoldur. Eğer `if` doğru değilse Python otomatik olarak `else` yoluna girer.", "task": "if 1 > 5: pass\n___: print('H')", "check": lambda c, o, i: "else" in c, "solution": "else:", "hint": "else: yaz."},
        {"msg": "**elif:** Birden fazla farklı şartı denetlemek için kullanılır. 'Eğer bu değilse, şuna bak' mantığıyla yukarıdan aşağıya taranır.", "task": "if p < 50: pass\n___ p > 50: pass", "check": lambda c, o, i: "elif" in c, "solution": "elif", "hint": "elif kullan."},
    ...
    {"module_title": "4. Otomasyon: For ve While Döngüleri", "exercises": [
        ...
        {"msg": "**While:** Belirli bir şart 'True' olduğu sürece sonsuz dönebilir. Döngüden çıkmak için içeride şartı bozacak bir işlem olmalıdır.", "task": "___ i < 5: print(i); i += 1", "check": lambda c, o, i: "while" in c, "solution": "while", "hint": "while yaz."},
        ...
        {"msg": "**continue:** O adımı pas geçip döngünün başına geri döner.", "task": "for i in range(5): if i==2: ___", "check": lambda c, o, i: "continue" in c, "solution": "continue", "hint": "continue."},
    ...
    {"module_title": "7. Nesneler: OOP Dünyası", "exercises": [
        ...
        {"msg": "**self:** Nesnenin kendisini temsil eden gizli parametredir.", "task": "def ses(___): pass", "check": lambda c, o, i: "self" in c, "solution": "self", "hint": "self yaz."},
    ...
    {"module_title": "8. Kalıcılık: Dosya Yönetimi", "exercises": [
        {"msg": "**open():** Dosya açmak için kullanılır. **'w'** yazma modudur.", "task": "f = ___('n.txt', '___')", "check": lambda c, o, i: "open" in c and "w" in c, "solution": "open, w", "hint": "open ve w."},
        {"msg": "**.write():** Veriyi dosyaya kalıcı mühürler.", "task": "f.___('X')", "check": lambda c, o, i: "write" in c, "solution": "write", "hint": "write."},
        {"msg": "**'r':** Dosyayı okumak için açma modudur.", "task": "f = open('a.txt', '___')", "check": lambda c, o, i: "r" in c, "solution": "r", "hint": "r koy."},
...
with col_main:
    if st.session_state.is_logged_in:
        ...
        st.markdown(f'<div class="user-header-box"><div><div class="info-label">AKADEMİ ÖĞRENCİSİ</div><div class="info-value">👤 {st.session_state.student_name} ({st.session_state.student_class})</div></div><div style="text-align:center;"><div class="info-label">RÜTBE</div><div class="info-value">{curr_r}</div></div><div style="text-align:right;"><div class="info-label">TOPLAM PUAN</div><div class="score-badge">⭐ {st.session_state.total_score}</div></div></div>', unsafe_allow_html=True)
    ...
    if not is_review and st.session_state.fail_count < 4 and not st.session_state.exercise_passed:
        # Editör key'ine her adımı ve hata sayısını bağlıyoruz ki her butona basıldığında temiz veri gelsin
