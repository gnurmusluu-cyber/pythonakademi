...
st.markdown("""
    ...
    .quest-bar { height: 18px; background: #e2e8f0; border-radius: 10px; margin: 10px 0; overflow: hidden; position: relative; }
    ...
    [data-testid="stTextInput"] { border: 2.5px solid #3a7bd5 !important; border-radius: 10px; }
    </style>
    """, unsafe_allow_html=True)

# --- 3. TEKNİK: MACBOOK GIF ÇÖZÜMÜ (BASE64) ---
...
def show_pito_gif(width=280):
    ...
    base64_str = get_base64_encoded_image(gif_path)
    
    if base64_str:
        st.markdown(f'<div style="display: flex; justify-content: center;"><img src="data:image/gif;base64,{base64_str}" width="{width}"></div>', unsafe_allow_html=True)
    else:
...
# --- 5. LİDERLİK TABLOSU (SAĞDA SABİT) ---
...
with col_app:
    ...
    with c_box_i: show_pito_gif(280)
    with c_box_m:
