...
st.markdown("""
    ...
    /* ACE EDITOR: APPLY BUTONUNU CSS İLE SİL */
    [data-testid="stAceApplyButton"], .ace-apply-button { display: none !important; visibility: hidden !important; }
...
with col_stats:
    ...
    if not db_current.empty:
        ...
        top_c = class_stats.sort_values(by="Puan", ascending=False).head(1).iloc[0]
        st.markdown(f'''<div style="background: linear-gradient(135deg, #FFD700, #F59E0B); color: black; border-radius: 15px; padding: 20px; text-align: center; font-weight: 900; margin-bottom: 25px;">🥇 LİDER SINIF: {top_c["Sınıf"]}</div>''', unsafe_allow_html=True)
        ...
        with t1:
            if st.session_state.is_logged_in:
                ...
                for _, r in my_c.iterrows(): st.markdown(f'<div class="ranking-card" style="background:white; padding:10px; margin-bottom:5px; border-radius:10px; border-left:5px solid #3a7bd5;"><b>{r["Öğrencinin Adı"]}</b><br>{int(r["Puan"])} PT</div>', unsafe_allow_html=True)
        with t2:
            for _, r in db_current.sort_values(by="Puan", ascending=False).head(10).iterrows(): st.markdown(f'<div class="ranking-card" style="background:white; padding:10px; margin-bottom:5px; border-radius:10px; border-left:5px solid #3a7bd5;"><b>{r["Öğrencinin Adı"]}</b> ({r["Sınıf"]})<br>{int(r["Puan"])} PT</div>', unsafe_allow_html=True)

# --- 6. DERİN MÜFREDAT VE ADIMLAR ---
training_data = [
    {"module_title": "1. İletişim: print() ve Metin Dünyası", "exercises": [
        {"msg": "**Pito'nun Notu:** Python'ın dünyayla konuştuğu tek kapı `print()` fonksiyonudur. Metinleri (String) mutlaka tırnak (' ') içine almalısın. Tırnaklar Python'a 'buradaki ifadeyi olduğu gibi yansıt' der.\n\n**GÖREV:** Editor içine tam olarak **'Merhaba Pito'** metnini tırnaklar içerisinde yaz!", "task": "print('___')", "check": lambda c, o, i: "Merhaba Pito" in o, "solution": "print('Merhaba Pito')", "hint": "Metnin başına ve sonuna tek (') tırnak koy."},
        {"msg": "**Sayılar (Integers):** Sayılar tırnak gerektirmez. Eğer bir sayıya tırnak koyarsan Python onu sayı değil, bir 'yazı' (String) olarak görür ve matematik yapamaz.\n\n**GÖREV:** Boşluğa tırnak kullanmadan sadece **100** sayısını yaz.", "task": "print(___)", "check": lambda c, o, i: "100" in o, "solution": "print(100)", "hint": "Rakamları tırnaksız yazmalısın."},
        {"msg": "**Virgül Operatörü:** Virgül (`,`) farklı veri tiplerini aynı satırda birleştirir ve otomatik boşluk koyar.\n\n**GÖREV:** 'Puan:' metni ile **100** sayısını yan yana bas.", "task": "print('Puan:', ___)", "check": lambda c, o, i: "100" in o, "solution": "print('Puan:', 100)", "hint": "Virgülden sonra 100 yaz."},
    ...
    {"module_title": "2. Hafıza: Değişkenler ve input()", "exercises": [
        {"msg": "**Değişkenler:** RAM'deki isimlendirilmiş kutulardır. `=` işareti bir 'atama operatörü'dür.\n\n**GÖREV:** `yas` ismindeki kutuya sayısal olarak **15** değerini ata.", "task": "yas = ___", "check": lambda c, o, i: i.get('yas') == 15, "solution": "yas = 15", "hint": "Eşittir işaretinden sonra 15 yaz."},
        ...
        {"msg": "**Casting:** Sayıları metne çevirmek için `str()` fonksiyonunu kullanırız.", "task": "print(___(10))", "check": lambda c, o, i: "str" in c, "solution": "print(str(10))", "hint": "str(değişken) formunu kullan."},
        {"msg": "**int():** `input()` verisini tam sayıya çevirmek için `int()` kullanılır.", "task": "n = ___(___('S: '))", "check": lambda c, o, i: "int" in c and "input" in c, "solution": "n = int(input('S: '))", "hint": "int(input()) yapısını kur."},
        {"msg": "**İsimlendirme:** Değişken isimlerinde rakamla başlamamalı ve boşluk olmamalıdır.", "task": "isim = '___'", "check": lambda c, o, i: i.get('isim') == 'Pito', "solution": "isim = 'Pito'", "hint": "Tırnaklar içine Pito yaz."}
    ]},
    # ... (Diğer 6 modülün derin içerikleri aynı yapıda devam eder) ...
    ...
    {"module_title": "5. Listeler: Veri Sepeti", "exercises": [{"msg": "**append**:", "task": "L.____(10)", "check": lambda c,o,i: "append" in c, "solution": "append", "hint": "append"} for _ in range(5)]},
...
with col_main:
    ...
    if not st.session_state.is_logged_in:
        ...
        with c2:
            st.markdown('<div class="pito-bubble" style="margin-top: 60px;">Merhaba Geleceğin Yazılımcısı!<br><br>Ben <b>Pito</b>. Numaranı gir ve mühürlü dünyaya katıl!</div>', unsafe_allow_html=True)
        ...
        if st.button("🔍 Kodumu Kontrol Et", use_container_width=True):
            ...
            try:
                ...
                if curr_ex.get('check', lambda c,o,i: True)(code, out, mock_env):
                    st.session_state.update({'feedback_msg': "✅ Tebrikler! Harika bir iş çıkardın. Bir sonraki adıma geçebilirsin!", 'exercise_passed': True, 'pito_emotion': 'mutlu'})
