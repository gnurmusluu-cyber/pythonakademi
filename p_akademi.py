    ...
    with st.sidebar:
        ...
        with tab_c:
            if st.session_state.get('is_logged_in'):
                my_class_df = df[df["Sınıf"] == st.session_state.student_class].sort_values(by="Puan", ascending=False).head(5)
                for _, r in my_class_df.iterrows():
...
training_data = [
    ...
    {"module_title": "2. Hafıza: Değişkenler ve input()", "exercises": [
        {"msg": "**Pito'nun Notu:** Değişkenler bilgisayarın hafızasındaki kutulardır. `=` işareti bir 'atama operatörü'dür ve sağdaki değeri soldaki kutunun içine koyar.\n\n**Görev:** `yas` değişkenine sayısal olarak **15** değerini ata.", "task": "yas = ___", "check": lambda c, o, i: "15" in c, "solution": "yas = 15", "hint": "Eşittir işaretinden sonra sadece değeri yazmalısın."},
        {"msg": "**Pito'nun Notu:** Metin (String) verilerini saklarken tırnak kullanmalısın. İsimlerde rakamla başlamamaya ve boşluk kullanmamaya dikkat etmelisin.\n\n**Görev:** `isim` değişkenine **'Pito'** metnini ata.", "task": "isim = '___'", "check": lambda c, o, i: "Pito" in o, "solution": "isim = 'Pito'", "hint": "Metni tırnaklar içerisine tam olarak Pito şeklinde yaz."},
    ...
    {"module_title": "6. Modülerlik: Fonksiyonlar ve Sözlükler", "exercises": [
        ...
        {"msg": "**Pito'nun Notu:** **Sözlükler (Dictionary)**, `{anahtar: değer}` çiftleriyle çalışır. Tıpkı rehberdeki bir isim ve ona ait telefon numarası gibi.\n\n**Görev:** 'ad' anahtarına karşılık gelen değer boşluğuna tırnaklar içinde **'Pito'** yaz.", "task": "d = {'ad': '___'}\nprint(d['ad'])", "check": lambda c, o, i: "Pito" in o, "solution": "d = {'ad': 'Pito'}", "hint": "Tırnakların arasına tam istediğim ismi yerleştir."},
        {"msg": "**Pito'nun Notu:** **Tuple (Demet)**, listeye çok benzer ama parantez `()` ile kurulur. En önemli farkı 'immutability' yani içeriğinin asla değiştirilemez olmasıdır.\n\n**Görev:** Boşluğa sayısal olarak **1** yazarak (1, 2) demetini oluştur.", "task": "t = (___, 2)\nprint(t)", "check": lambda c, o, i: "1" in c, "solution": "t = (1, 2)", "hint": "Sadece eksik olan 1 rakamını koyman yeterli."},
        {"msg": "**Pito'nun Notu:** `.keys()` metodu sözlükteki tüm etiketleri (anahtarları) bize bir liste halinde sunar.\n\n**Görev:** Boşluğa **keys** yazarak sözlükteki anahtarları ekrana çek.", "task": "d = {'a':1}\nprint(d.___())", "check": lambda c, o, i: "keys" in c, "solution": "d.keys()", "hint": "Anahtarlar kelimesinin İngilizcesini düşün: keys."},
...
with col_main:
    ...
    # --- GERİ BİLDİRİM PANELİ (MÜHÜRLÜ KONUM: KOD PANELİ ÜSTÜ) ---
    ...
    def run_pito_code(c, user_input="Pito"):
        ...
        try:
            mock_globals = {"print": print, "input": lambda p: str(user_input), "int": int, "str": str, "len": len, "open": open, "range": range, "s": 10, "L": [10, 20], "d":{'ad':'Pito'}, "t":(1,2)}
            exec(c, mock_globals); out = new_stdout.getvalue(); sys.stdout = old_stdout
            return out
        except Exception as e:
            sys.stdout = old_stdout; return f"❌ Python Hatası: {e}"
