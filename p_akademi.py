...
def load_resources():
    try:
        ...
        # Pito Mesaj Bankasını Yükle
        with open('messages.json', 'r', encoding='utf-8') as f:
            st.session_state.pito_messages = json.load(f)
    except Exception as e:
        st.error(f"⚠️ Kritik Kaynak Hatası: JSON dosyaları (style/messages) bulunamadı! {e}")
...
def init_supabase():
    ...
    except: st.error("⚠️ Supabase bağlantısı kurulamadı!"); st.stop()
...
def liderlik_tablosu_goster(user_sinif=None):
    ...
    try:
        ...
        # Sınıf ve Lig sekmeleri de benzer mantıkla çalışır
    except: st.write("Liderlik tablosu güncelleniyor...")

# --- 5. İLERLEME VE VERİTABANI KAYDI ---
...
if st.session_state.user is None:
    ...
    with col_in:
        ...
        if not st.session_state.show_reg and st.session_state.temp_user is None:
            ...
            st.markdown("<div class='pito-notu'>👋 Seni daha önce görmemiştim! Kaydını yapalım genç yazılımcı.</div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<div class='pito-notu'>👋 <b>Selam {st.session_state.temp_user['ad_soyad']}!</b> <br> Bu sen misin?</div>", unsafe_allow_html=True)
            c1, c2 = st.columns(2)
    ...
    if st.session_state.in_review:
        ...
        with cl:
            rn, rc = rütbe_belirle(u['toplam_puan'])
            ...
            # Puanlama: $XP = \max(0, 20 - (hata \times 5))$
            ...
            # Pito Etkileşim
            cp1, cp2 = st.columns([1, 2]); with cp1: pito_gorseli_yukle(st.session_state.pito_mod)
            with cp2:
