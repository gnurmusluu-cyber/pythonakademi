...
st.markdown("""
    ...
    /* ACE EDITOR: APPLY BUTONUNU TAMAMEN GİZLE */
    iframe { border: none !important; }
    .ace-apply-button, [data-testid="stAceApplyButton"] { display: none !important; visibility: hidden !important; }
...
with col_stats:
    ...
    if not db_current.empty:
        ...
        st.markdown(f'''<div style="background: linear-gradient(135deg, #FFD700, #F59E0B); color: black; border-radius: 15px; padding: 20px; text-align: center; font-weight: 900; margin-bottom: 25px; box-shadow: 0 10px 20px rgba(245,158,11,0.4);">👑 ŞAMPİYON: {top_class["Sınıf"]}</div>''', unsafe_allow_html=True)
        ...
        with t1:
            if st.session_state.is_logged_in:
                ...
                for _, r in my_c.iterrows(): st.markdown(f'<div class="ranking-card"><div><b>{r["Öğrencinin Adı"]}</b><br><small>{r["Rütbe"]}</small></div><div style="color:#3a7bd5; font-weight:900;">{int(r["Puan"])} PT</div></div>', unsafe_allow_html=True)
        with t2:
            for _, r in db_current.sort_values(by="Puan", ascending=False).head(10).iterrows(): st.markdown(f'<div class="ranking-card"><div><b>{r["Öğrencinin Adı"]}</b> ({r["Sınıf"]})<br><small>{r["Rütbe"]}</small></div><div style="color:#3a7bd5; font-weight:900;">{int(r["Puan"])} PT</div></div>', unsafe_allow_html=True)

# --- 6. MUTLAK VE DERİN MÜFREDAT (40 DERİN ADIM) ---
training_data = [
    {"module_title": "1. İletişim: print() ve Metin Dünyası", "exercises": [
        {"msg": "**Pito'nun Notu:** Python'ın dünyayla konuştuğu tek kapı `print()` fonksiyonudur. Ekrana yazacağın metinleri (yazıları) mutlaka tırnak (' ') içine almalısın. Tırnaklar bilgisayara 'buradaki ifadeyi olduğu gibi yansıt' der.\n\n**GÖREV:** Editor içine tam olarak **'Merhaba Pito'** metnini tırnaklar içerisinde yaz!", "task": "print('___')", "check": lambda c, o, i: "Merhaba Pito" in o, "solution": "print('Merhaba Pito')", "hint": "Metnin başına ve sonuna tek (') tırnak koy."},
    ...
    {"module_title": "6. Modülerlik: Fonksiyonlar ve Sözlükler", "exercises": [
        ...
        {"msg": "**Tuple:** Değiştirilemeyen (mühürlü) listelerdir.", "task": "t = (___, 2)", "check": lambda c, o, i: "1" in str(i.get('t','')), "solution": "1", "hint": "1 yaz."},
        {"msg": "**.keys():** Sözlükteki tüm etiketleri (anahtarları) verir.", "task": "d.___()", "check": lambda c, o, i: "keys" in c, "solution": "keys", "hint": "keys."},
        {"msg": "**return:** Fonksiyonun sonucunu dışarı fırlatır.", "task": "def f(): ___ 5", "check": lambda c, o, i: "return" in c, "solution": "return", "hint": "return."}
    ]},
    ...
    {"module_title": "8. Kalıcılık: Dosya Yönetimi", "exercises": [
        {"msg": "**open():** Dosyayı 'w' (write - yazma) modunda açar.", "task": "f = ___('a.txt', '___')", "check": lambda c, o, i: "open" in c and "w" in c, "solution": "open, w", "hint": "open ve w."},
...
with col_main:
        ...
        with c2:
            ...
            if in_no and in_no.isdigit():
                ...
                if not user_data.empty:
                    ...
                    st.info(f"🔍 **{row['Öğrencinin Adı']}**, Hoş geldin! {'🎓 Mezuniyet mühürlendi.' if int(row['Mevcut Modül']) >= 8 else 'Devam edelim.'}")
    ...
    if not is_review and st.session_state.fail_count < 4 and not st.session_state.exercise_passed:
        # ACE EDİTÖR: Auto Update True + Manual Check = Kesin Çözüm
            ...
            if "___" in code:
                ...
                try:
                    ...
                    if curr_ex.get('check', lambda c,o,i: True)(code, out, mock_env):
                        st.session_state.update({'feedback_msg': "✅ Harika! Görevi mühürledin.", 'exercise_passed': True, 'pito_emotion': 'mutlu'})
