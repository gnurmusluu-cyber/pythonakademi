...
# --- 1. SİBER-ZIRH: GÖZ DOSTU NEON ENERJİ (#ADFF2F) CSS ---
st.set_page_config(
...
st.markdown("""
    ...
    /* MODÜL BAŞLIĞI (NEON ÜSTÜNE SİYAH) */
    .hero-panel { 
        background-color: #ADFF2F !important; 
        padding: 20px !important; border-radius: 12px !important; margin-bottom: 25px !important;
        border: 2px solid #FFFFFF !important;
        box-shadow: 0 0 15px rgba(173, 255, 47, 0.3) !important;
    }
    .hero-panel h3 { color: #000000 !important; font-weight: 900 !important; margin: 0 !important; text-transform: uppercase; }
    .hero-panel p { color: #000000 !important; font-weight: 800 !important; margin: 0 !important; }

    /* ÇÖZÜM BLOĞU BAŞLIĞI (NEON ÜSTÜNE SİYAH) */
...
def pito_notu_uret(mod, ad="Genç Yazılımcı"):
    notlar = {
        "merhaba": f"Selam {ad}! Bugün Python'da hangi kapıyı aralayacağız?",
        "basari": f"Vay canına {ad}! Kodun saat gibi tıkır tıkır çalıştı.",
...
if st.session_state.user is None:
    ...
    if m_idx >= total_m: # --- MEZUNİYET ---
        ...
        with col2:
            ...
            st.markdown(f"""<div class="diploma-box"><h1 style="color:#FFD700;">🏆 BİLGE SERTİFİKASI</h1><h2>{u['ad_soyad'].upper()}</h2><p>Akademiyi Başarıyla Tamamladın!</p></div>""", unsafe_allow_html=True)
        ...
        with cl:
            ...
            with cp2:
                # --- HATA YÖNETİMİ VE TONLAMA (DÜZELTİLDİ) ---
                ad_kisa = u['ad_soyad'].split()[0]
                if st.session_state.error_count == 1: 
                    st.error(f"🚨 Pito: {ad_kisa}, bu senin 1. hatan arkadaşım. Kodu bir daha kontrol et!"); st.session_state.pito_mod = "hata"
                elif st.session_state.error_count == 2: 
                    st.error(f"🚨 Pito: {ad_kisa}, bu 2. hatan oldu. Tırnaklara veya parantezlere dikkat et!"); st.session_state.pito_mod = "hata"
                elif st.session_state.error_count == 3: 
                    st.warning(f"💡 Pito İpucu: {egz['ipucu']}"); st.session_state.pito_mod = "hata"
                elif st.session_state.error_count >= 4: 
                    st.error("🚫 Maksimum hata! Bu seferlik çözümü inceleyelim."); st.session_state.pito_mod = "dusunuyor"
