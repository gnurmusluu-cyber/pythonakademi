...
st.markdown("""
    ...
    .resume-btn > button { background: linear-gradient(45deg, #28a745, #85e085) !important; }
    .review-btn > button { background: linear-gradient(45deg, #ffc107, #ffdb4d) !important; color: black !important; }
    </style>
...
def auto_save_progress():
    try:
        ...
        # Sadece ileri gitmeyi kurgula
        df_all = get_db()
        user_row = df_all[df_all["Okul No"] == no]
        ...
        if not user_row.empty:
            ...
            if st.session_state.current_module > db_m:
                ...
                st.session_state.db_exercise = max(db_e, st.session_state.current_exercise)
        else:
            st.session_state.db_module, st.session_state.db_exercise = st.session_state.current_module, st.session_state.current_exercise
...
if 'is_logged_in' not in st.session_state:
    for k, v in {'student_name': "", 'student_no': "", 'student_class': "", 'completed_modules': [False]*8, 
                 ...
                 'scored_exercises': set(), 'db_module': 0, 'db_exercise': 0, 'is_logged_in': False, 'recovery_mode': False}.items():
        st.session_state[k] = v

PITO_IMG = "assets/pito.png"

# --- 4. GİRİŞ EKRANI (AKILLI KARŞILAMA) ---
    ...
    with col_mid:
        ...
        if in_no:
            ...
            if not user_data.empty:
                ...
                st.info(f"Puanın: {row['Puan']} | Kaldığın Yer: Modül {int(row['Mevcut Modül'])+1}")
                
                c1, c2 = st.columns(2)
                with c1:
                    if st.button("🚀 Kaldığın Yerden Devam Et", use_container_width=True):
                        ...
                        st.rerun()
                with c2:
                    if st.button("📚 Geçmişi İncele", use_container_width=True):
                        st.session_state.student_no, st.session_state.student_name, st.session_state.student_class = row["Okul No"], row["Öğrencinin Adı"], row["Sınıf"]
                        st.session_state.total_score, st.session_state.db_module, st.session_state.db_exercise = int(row["Puan"]), int(row["Mevcut Modül"]), int(row["Mevcut Egzersiz"])
                ...
                st.warning("Numaranı ilk kez giriyorsun. Lütfen kayıt bilgilerini tamamla:")
...
with col_main:
    ...
    with c_img: st.image(PITO_IMG if os.path.exists(PITO_IMG) else "https://img.icons8.com/fluency/200/robot-viewer.png", width=160)
    with c_msg:
        st.info(f"##### 🗣️ Pito:\n{curr_ex['msg']}")
        st.caption(f"Adım: {e_idx + 1}/5 {'🔒 (Tamamlandı)' if is_locked else '🚀 Aktif Görev'}")
    ...
    if not is_locked:
        if st.button("🔍 Kontrol Et", use_container_width=True):
            ...
            try:
                ...
                st.code(out if out else "Başarılı!")
...
with col_side:
    ...
    if not df_class.empty:
        ...
        for i, (_, r) in enumerate(df_lb.iterrows()):
