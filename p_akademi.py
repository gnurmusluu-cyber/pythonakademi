...
def force_save():
    """Mükerrer kaydı siler ve tek satırda günceller."""
    try:
        ...
        df = get_db()
        # Okul No aynı olanları temizle (Primary Key mantığı)
        df = df[df["Okul No"] != no]
        
        new_row = pd.DataFrame([[
            no, st.session_state.student_name, st.session_state.student_class,
            score, rank, progress, st.session_state.current_module,
            st.session_state.current_exercise, datetime.now().strftime("%H:%M:%S")
...
if st.session_state.student_name == "":
    ...
    with col_mid:
        st.markdown('<div class="pito-bubble">Merhaba Arkadaşlar! Ben <b>Pito</b>. Numaranı gir ve kaldığın yerden devam et!</div>', unsafe_allow_html=True)
...
training_data = [
    {"module_title": "1. Giriş ve Çıktı", "exercises": [
        ...
        {"msg": "Puan: 100 yazdır.", "task": "print('Puan:', ___)", "check": lambda c, o: "Puan: 100" in o},
