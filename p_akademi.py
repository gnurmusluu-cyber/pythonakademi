...
st.markdown("""
    ...
    [data-testid="stExpander"] summary p { color: #000000 !important; font-weight: 900 !important; }
...
if st.session_state.user is None:
    ...
    with col_l:
        ...
        if not st.session_state.show_reg and st.session_state.temp_user is None:
            ...
            if st.button("Kaydı Tamamla arkadaşım! ✨"):
                ...
                supabase.table("kullanicilar").insert(new_u).execute()
                st.session_state.user = new_u; st.session_state.show_reg = False; st.rerun()
        
        elif st.session_state.temp_user:
            st.markdown(f"<div class='pito-notu'>👋 Selam {st.session_state.temp_user['ad_soyad']}! Bu sen misin genç yazılımcı?</div>", unsafe_allow_html=True)
            c1, c2 = st.columns(2)
    ...
    if m_idx >= total_m:
        ...
        with cl:
            ...
            st.markdown(f"<div class='hero-panel'><h3>🚀 {modul['modul_adi']}</h3><p>{u['ad_soyad']} | <span class='rank-badge {r_cls}' style='background:black; color:#ADFF2F;'>{r_ad}</span> | {int(u['toplam_puan'])} XP</p></div>", unsafe_allow_html=True)
            ...
            if not st.session_state.cevap_dogru and st.session_state.error_count < 4:
                ...
                if st.session_state.error_count in [1, 2]: st.error(f"🚨 Pito: {ad_k}, bu senin {st.session_state.error_count}. hatan arkadaşım!")
