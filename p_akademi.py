...
st.markdown("""
    ...
    /* 2. Metin Rengi ve Görünürlük (Koyu Lacivert) */
    ...
    /* 3. INPUT VE SELECTBOX KESIN ÇÖZÜM (Siyah kutu hatasını giderir) */
    ...
    .rank-tag { display: inline-block; background: #3a7bd5; color: white !important; padding: 2px 8px; border-radius: 10px; font-size: 0.75rem; margin-top: 5px; }
...
# --- 6. MEB KAYNAKLI EKSİKSİZ MÜFREDAT ---
training_data = [
...
with col_main:
    ...
    # --- KRİTİK: ÇÖZÜM SADECE İNCELEME MODUNDA VE OKUNAKLI ---
    if is_locked:
    ...
    if not is_locked:
        ...
        if st.button("🔍 Kodumu Kontrol Et"):
            ...
            try:
                # İnceleme modunda ValueError veren 'Örnek Veri' yerine '10' sabiti kullanıldı
                ...
                if curr_ex['check'](code, out) and "___" not in code:
                    ...
                    if f"{m_idx}_{st.session_state.current_exercise}" not in st.session_state.scored_exercises:
                        ...
                        force_save()
                else: st.session_state.update({'pito_emotion': "pito_hata", 'feedback_type': "error", 'feedback_msg': "Yanıtın hatalı veya eksik."})
