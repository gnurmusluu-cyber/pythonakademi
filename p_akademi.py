...
st.markdown("""
    ...
    .hero-panel { background-color: #ADFF2F !important; padding: 20px !important; border-radius: 12px !important; margin-bottom: 20px !important; border: 2px solid #FFFFFF !important; }
...
if st.session_state.user is None:
    ...
    with col_l:
        ...
        if not st.session_state.show_reg and st.session_state.temp_user is None:
            ...
            st.markdown(f"<div class='pito-notu'>👋 <b>Yeni bir yüz!</b> <br> {st.session_state.user_num} numarasını ilk kez görüyorum. Kaydını hemen yapalım!</div>", unsafe_allow_html=True)
            new_name = st.text_input("Adın ve Soyadın:")
            new_class = st.selectbox("Sınıfın:", ["9-A", "9-B", "10-A", "10-B", "11-A", "11-B", "12-A", "12-B"])
            if st.button("Kaydı Tamamla ve Başla! ✨"):
                if new_name:
                    ...
                    supabase.table("kullanicilar").insert(new_user).execute()
                    st.session_state.user = new_user
                    st.session_state.show_reg = False
                    st.rerun()
                else: st.warning("Lütfen adını yaz arkadaşım!")
    ...
    if m_idx >= total_m: # --- MEZUNİYET ---
        ...
        # MODÜL İÇİ GÖREV İLERLEME ÇUBUĞU
        ...
        with cl:
            ...
            with cp2:
                ...
                if st.session_state.error_count == 1: st.error(f"🚨 Pito: {ad_kisa}, bu senin 1. hatan arkadaşım!"); st.session_state.pito_mod = "hata"
                elif st.session_state.error_count == 2: st.error(f"🚨 Pito: {ad_kisa}, bu 2. hatan oldu. Sakinleş!"); st.session_state.pito_mod = "hata"
                ...
                st.markdown(f"<div class='pito-notu'>💬 <b>Pito:</b> {ad_kisa}, beraber halledeceğiz!</div>", unsafe_allow_html=True)
