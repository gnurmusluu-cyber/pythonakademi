...
# Mobil uyumluluk ve şık görünüm için CSS
st.markdown("""
    <style>
...
# Sizin paylaştığınız tablo linki entegre edildi
...
def auto_save_score():
    """Öğrenci her doğru cevap verdiğinde puanı otomatik olarak buluta işler."""
    try:
...
# --- 5. EĞİTİM VERİLERİ (8 MODÜL EKSİKSİZ) ---
def get_rank(score):
    ...
    try:
        ...
        if curr_ex['check'](code, output) and "___" not in code:
            ...
            if ex_key not in st.session_state.scored_exercises:
                ...
                auto_save_score() # Puan anlık olarak Sheets'e kaydedilir
