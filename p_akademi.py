...
# --- 2. SUPABASE BAĞLANTI KONTROLÜ ---
try:
    url = st.secrets["supabase"]["url"]
    key = st.secrets["supabase"]["key"]
    supabase: Client = create_client(url, key)
except Exception as e:
    st.error("❌ Secrets Hatası: Supabase bilgileri bulunamadı!")
    st.info("Lütfen Streamlit Secrets bölümüne [supabase] başlığıyla URL ve KEY ekleyin.")
    st.stop()
...
def ilerleme_kaydet(puan, kod, egz_id, n_id, n_m):
    try:
        ...
        supabase.table("egzersiz_kayitlari").insert({
            "ogrenci_no": st.session_state.user['ogrenci_no'], 
            "egz_id": str(egz_id), "alinan_puan": puan, "basarili_kod": kod
        }).execute()
...
if st.session_state.user is None:
    ...
    with col_mid:
        ...
        if st.session_state.temp_user is None:
            ...
            if numara > 0 and st.button("Akademi Kapısını Aç 🔑"):
                res = supabase.table("kullanicilar").select("*").eq("ogrenci_no", numara).execute()
                if res.data:
                    ...
                    if st.button("Kaydı Tamamla 🎓") and y_ad:
                        new_user = {"ogrenci_no": int(numara), "ad_soyad": y_ad, "sinif": y_sin}
    ...
    with col_main:
        ...
        if m_idx >= len(mufredat['pito_akademi_mufredat']):
            ...
            st.markdown('<div class="pito-notu" style="text-align:center;">Tüm görevleri tamamladın!</div>', unsafe_allow_html=True)
            ...
            if not st.session_state.cevap_dogru and st.session_state.error_count < 4:
                k_in = st.text_area("Kodunu Yaz:", value=egz['sablon'], height=200, key="editor_v24")
