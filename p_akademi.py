...
if not st.session_state.is_logged_in:
    ...
    with col_mid:
        st.markdown('<div class="pito-bubble">Merhaba Geleceğin Yazılımcısı! Ben <b>Pito</b>. Python dünyasına giriş yapmaya hazır mısın? Sana sadece kod yazmayı değil, bir yazılımcı gibi düşünmeyi öğreteceğim. Başlayalım mı?</div>', unsafe_allow_html=True)
        st.image(PITO_IMG if os.path.exists(PITO_IMG) else "https://img.icons8.com/fluency/180/robot-viewer.png", width=180)
        in_no = st.text_input("Okul Numaran (Giriş anahtarın):", key="login_field").strip()
        if in_no and in_no.isdigit():
            df = get_db()
            user_data = df[df["Okul No"] == in_no]
            if not user_data.empty:
                row = user_data.iloc[0]
                st.info(f"🚀 Sistem seni tanıdı: **{row['Öğrencinin Adı']}**. Kaldığın yerden devam edebilirsin.")
                if st.button("✅ Maceraya Geri Dön"):
                ...
                in_name = st.text_input("Adın Soyadın (Sertifikan için):")
                in_class = st.selectbox("Sınıfın:", SINIFLAR)
                if st.button("Kayıt Ol ve Python'a Başla ✨") and in_name:
...
training_data = [
    {"module_title": "1. Merhaba Python: İletişim Kurmak", "exercises": [
        ...
        {"msg": "**Konu Özeti:** Virgül (`,`) Python'da sihirli bir birleştiricidir. Farklı veri türlerini (metin ve sayı gibi) aynı satırda yan yana yazdırmamızı sağlar.\n\n**Görev:** Önce **'Puan:'** metnini, hemen yanına ise **100** sayısını yazdır.", "task": "print('Puan:', ___)", "check": lambda c, o: "100" in o, "solution": "print('Puan:', 100)", "has_output": True},
        {"msg": "**Konu Özeti:** İyi bir yazılımcı, koduna notlar bırakır. `#` işareti 'yorum satırı' demektir. Bilgisayar bu satırı görmezden gelir ama bizler için bir rehberdir.\n\n**Görev:** Bir `#` işareti ekleyerek bu satırı bilgisayarın okumadığı bir yorum satırına dönüştür.", "task": "___ bu bir yorumdur", "check": lambda c, o: "#" in c, "solution": "# bu bir yorumdur", "has_output": False},
        {"msg": "**Konu Özeti:** Metinlerin içinde alt satıra geçmek için `\\n` (new line) karakteri kullanılır. Bu, metni daha okunabilir yapar.\n\n**Görev:** 'Üst' kelimesinden sonra alt satıra geçip 'Alt' yazmasını sağlayacak boşluğu doldur.", "task": "print('Üst' + '___' + 'Alt')", "check": lambda c, o: "\n" in o, "solution": "print('Üst\\nAlt')", "has_output": True}
    ]},
    {"module_title": "2. Değişkenler ve input(): Veriyi Hafızada Tutmak", "exercises": [
        {"msg": "**Konu Özeti:** Değişkenler, verileri sakladığımız isimlendirilmiş kutulardır. `=` işareti atama yapar. Bellekte (RAM) yer ayırırız.\n\n**Görev:** **yas** ismindeki değişkene (kutuya) **15** değerini koy.", "task": "yas = ___\nprint(yas)", "check": lambda c, o: "15" in o, "solution": "yas = 15\nprint(yas)", "has_output": True},
...
with col_main:
    ...
    if st.session_state.db_module >= 8:
        ...
        with c1:
            if st.button("🔄 Eğitimi Tekrar Al"):
                ...
                force_save(); st.rerun()
        with c2:
            if st.button("🏆 Listede Kal"): st.info("Başarın tarihe geçti!")
        st.divider()

    mod_titles = [f"{'✅' if st.session_state.completed_modules[i] else '📖'} Modül {i+1}" for i in range(8)]
    sel_mod = st.selectbox("Eğitim Modülü:", mod_titles, index=m_idx)
    ...
    with c_msg:
        ...
        st.caption(f"İlerleme: {e_idx + 1}/{len(training_data[st.session_state.current_module]['exercises'])} | " + ("🔒 Arşiv" if is_locked else f"🎁 Kazanılacak Puan: {st.session_state.current_potential_score} | ❌ Hata: {st.session_state.fail_count}/4"))
    ...
    if st.session_state.fail_count == 3 and not is_locked:
        ...
        st.error("❌ Son denemede de hata oluştu. Puan kazanamadın ama pes etmek yok! Sonraki adıma ilerleyebilirsin.")
    ...
    if is_locked:
        ...
        if st.session_state.fail_count < 4:
            ...
            if st.session_state.exercise_passed and st.session_state.last_output:
                st.markdown("**Kodun Çalıştı! Çıktı Şöyle:**")
                ...
                if "___" in code:
                    ...
                    if out.startswith("❌") or not curr_ex['check'](code, out):
                        ...
                        if st.session_state.fail_count >= 4:
                            ...
                            st.session_state.feedback_msg = f"❌ Maalesef, bu {st.session_state.fail_count}. hatalı girişin. Tekrar dene!"
                        st.rerun()
                    else:
