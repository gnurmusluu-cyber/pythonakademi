...
# --- MODERN UI CSS (BUTON VE GÖRÜNÜRLÜK MÜHÜRÜ) ---
st.markdown("""
    <style>
    ...
    .ace-apply-button, [data-testid="stAceApplyButton"], button[class*="ace"] { display: none !important; }
...
with col_stats:
    ...
    if not db_current.empty:
        df_c = db_current.copy()
        df_c["Puan"] = pd.to_numeric(df_c["Puan"], errors='coerce').fillna(0)
        top_c = df_c.groupby("Sınıf")["Puan"].sum().idxmax()
        ...
        with t1:
            if st.session_state.is_logged_in:
                my_c = df_c[df_c["Sınıf"] == st.session_state.student_class].sort_values(by="Puan", ascending=False).head(8)
        ...
        with t2:
            for _, r in df_c.sort_values(by="Puan", ascending=False).head(10).iterrows(): st.markdown(f'<div class="ranking-card" style="background:white; padding:10px; margin-bottom:5px; border-radius:10px; border-left:5px solid #3a7bd5;"><b>{r["Öğrencinin Adı"]}</b> ({r["Sınıf"]})<br>{int(r["Puan"])} PT</div>', unsafe_allow_html=True)

# --- 6. DERİN VE PEDAGOJİK MÜFREDAT (40 ADIM) ---
training_data = [
...
with col_main:
    ...
    if not st.session_state.exercise_passed and st.session_state.fail_count < 4:
        # Editör key'ine hata sayısını ve adımı bağlıyoruz (Senkronizasyon Fix)
        ...
        if st.button("🔍 Kodumu Kontrol Et", use_container_width=True):
            ...
            try:
                ...
                # BAŞARI DURUMU (fail_count ARTMADAN BAŞARIYA KİLİTLENİR)
                if curr_ex.get('check', lambda c,o,i: True)(code, out, mock_env):
                    st.session_state.update({
                        ...
                        'pito_emotion': 'mutlu',
                        'fail_count': 0 # Başarıda hata sıfırlanır
                    })
