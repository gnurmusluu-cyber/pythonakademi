...
# Liderlik Tablolarını Sidebar'da göster (Giriş öncesi dahil her zaman görünür)
...
# Veri tabanını oku ve liderlik tablolarını sidebar'da hazırla
db_current = get_db()
...
training_data = [
    ...
    {"module_title": "3. Karar Yapıları: If-Else Mantığı", "exercises": [
        {"msg": "**Pito'nun Notu:** Programların karar verme yeteneği `if` bloğuyla başlar. Eşitlik kontrolü yaparken `=` değil, mutlaka `==` kullanmalısın.\n\n**Görev:** Sayı 10'a eşitse 'OK' yazdıracak operatörü (**==**) boşluğa yaz.", "task": "if 10 ___ 10: print('OK')", "check": lambda c, o, i: "==" in c, "solution": "if 10 == 10:", "hint": "Mantıksal karşılaştırma için çift eşittir (==) şarttır."},
...
with col_main:
    ...
    # FIXED 4. HATA LOGİC: 4. hatada ipucu kaybolur, çözüm ve kilit mesajı gelir
        ...
        if st.session_state.fail_count < 4 and not st.session_state.exercise_passed:
            ...
            if st.button("🔍 Kodumu Kontrol Et", use_container_width=True):
                if "___" in code: st.session_state.feedback_msg = "⚠️ Pito bekliyor: Lütfen önce boşluğu doldur!"; st.rerun()
                    ...
                    try:
                        mock_globals = {"print": print, "input": lambda x: "10", "range": range, "s": 10, "L": [10, 20], "d":{'ad':'Pito'}, "t":(1,2)}
                        exec(code, mock_globals); out = new_stdout.getvalue(); sys.stdout = old_stdout
