...
# --- 3. HIZ VE GÖRÜNÜRLÜK MÜHÜRLERİ (CACHING & UI) ---
@st.cache_resource
def load_gif(name):
...
st.markdown("""
    ...
    .user-header-box {
        ...
        border-radius: 20px !important; padding: 15px 25px !important; margin-bottom: 20px !important;
...
def show_pito_gif(width=450):
    ...
    b64 = load_gif(emotion_map.get(st.session_state.pito_emotion, "pito_dusunuyor.gif"))
    if b64:
...
with col_stats:
    ...
    if not db_current.empty:
        ...
        st.markdown(f'''<div style="background: linear-gradient(135deg, #FFD700, #F59E0B); color: black; border-radius: 15px; padding: 20px; text-align: center; font-weight: 900; margin-bottom: 25px;">🥇 LİDER SINIF: {top_class["Sınıf"]}</div>''', unsafe_allow_html=True)
        ...
        with t2:
            for _, r in db_current.sort_values(by="Puan", ascending=False).head(10).iterrows(): st.markdown(f'<div class="ranking-card"><div><b>{r["Öğrencinin Adı"]}</b> ({r["Sınıf"]})</div><div style="color:#3a7bd5; font-weight:900;">{int(r["Puan"])}</div></div>', unsafe_allow_html=True)

# --- 6. MUTLAK VE DERİN MÜFREDAT (40 ADIM) ---
training_data = [
    {"module_title": "1. İletişim: print() ve Metin Dünyası", "exercises": [
        {"msg": "**Pito'nun Notu:** Python'ın dünyayla konuştuğu tek kapı `print()` fonksiyonudur. Ekrana yazacağın metinleri (yazıları) mutlaka tırnak (' ') içine almalısın. Tırnaklar bilgisayara 'buradaki ifadeyi olduğu gibi yansıt' mesajını verir.\n\n**GÖREV:** Editor içine tam olarak **'Merhaba Pito'** metnini tırnaklar içerisinde yaz!", "task": "print('___')", "check": lambda c, o, i: "Merhaba Pito" in o, "solution": "print('Merhaba Pito')", "hint": "Metnin başına ve sonuna tek (') tırnak koy."},
        {"msg": "**Sayılar (Integers):** Sayılar (tam sayılar) tırnak gerektirmezler. Eğer bir sayıya tırnak koyarsan Python onu sayı değil, bir 'yazı' (String) olarak görür ve üzerinde toplama/çıkarma gibi matematiksel işlemler yapamaz.\n\n**GÖREV:** Boşluğa tırnak kullanmadan sadece **100** sayısını yaz.", "task": "print(___)", "check": lambda c, o, i: "100" in o, "solution": "print(100)", "hint": "Sayısal değerleri tırnaksız yazmalısın."},
    ...
    {"module_title": "2. Hafıza: Değişkenler ve input()", "exercises": [
        ...
        {"msg": "**İsimlendirme:** Değişken isimlerinde rakamla başlamamaya ve boşluk kullanmamaya dikkat et! Python büyük-küçük harfe duyarlıdır (`yas` ve `Yas` farklıdır).\n\n**GÖREV:** `isim` değişkenine **'Pito'** metnini ata.", "task": "isim = ___", "check": lambda c, o, i: "Pito" in str(i.get('isim', '')), "solution": "isim = 'Pito'", "hint": "Metni tırnaklar içine Pito olarak yaz."}
    ]},
    {"module_title": "3. Mantık: Karar Yapıları (If-Else)", "exercises": [
        {"msg": "**Eşitlik Sorgusu:** Karar yapılarında `=` (atama) ile `==` (eşitlik sorgusu) çok farklıdır. İki değer birbirine tıpatıp aynı mı diye bakarken mutlaka çift eşittir kullanmalısın.", "task": "if 10 ___ 10: print('OK')", "check": lambda c, o, i: "==" in c, "solution": "if 10 == 10:\n    print('OK')", "hint": "== koy."},
        {"msg": "**B Planı:** `else:` şart sağlanmadığında devreye giren yoldur. Eğer `if` içindeki durum 'Yanlış' (False) ise Python otomatik olarak `else` yoluna girer.", "task": "if 1 > 5: pass\n___: print('Hata')", "check": lambda c, o, i: "else" in c, "solution": "else:", "hint": "else: yaz ve iki noktayı unutma."},
        {"msg": "**elif:** Birden fazla farklı şartı denetlemek için kullanılır. 'Eğer bu değilse, şuna bak' mantığıyla yukarıdan aşağıya taranır.\n\n**GÖREV:** Puan 50'den büyükse kontrolü için boşluğa **elif** yaz.", "task": "if p < 50: pass\n___ p > 50: print('G')", "check": lambda c, o, i: "elif" in c, "solution": "elif p > 50:", "hint": "elif kullan."},
        {"msg": "**and Operatörü:** Bu bağlaç iki tarafın da doğru (True) olmasını bekler. Biri bile yanlışsa blok çalışmaz. Çok sıkı bir denetçidir.", "task": "if 1==1 ___ 2==2: print('OK')", "check": lambda c, o, i: "and" in c, "solution": "and", "hint": "and anahtarını yerleştir."},
        {"msg": "**!= Operatörü:** 'Eşit değilse' anlamına gelir. Şartın gerçekleşmediği veya verilerin birbirinden farklı olduğu durumları yakalar.", "task": "if s ___ 0: print('Var')", "check": lambda c, o, i: "!=" in c, "solution": "!=", "hint": "!= koy."}
    ]},
    {"module_title": "4. Otomasyon: For ve While Döngüleri", "exercises": [
        {"msg": "**range:** `range(5)` komutu 0'dan 4'e kadar 5 sayı üretir. `for` döngüsü bu sayılarda adım adım ilerleyerek işlemi tekrarlar.", "task": "for i in ___(5): print(i)", "check": lambda c, o, i: "range" in c, "solution": "range", "hint": "range yaz."},
        {"msg": "**While:** Belirli bir şart 'True' olduğu sürece sonsuz dönebilir. Döngünün bitmesi için içeride şartı bozacak bir işlem (i += 1 gibi) olmalıdır.", "task": "___ i < 5: print(i); i += 1", "check": lambda c, o, i: "while" in c, "solution": "while", "hint": "while yaz."},
        {"msg": "**break:** Döngüyü anında bitiren acil çıkış kapısıdır. Şart sağlandığı an Python döngüden fırlar ve alt satırlara geçer.", "task": "for i in range(5): if i==2: ___", "check": lambda c, o, i: "break" in c, "solution": "break", "hint": "break."},
        {"msg": "**continue:** O anki adımı 'pas geçer' ve döngünün en başına geri döner. Altındaki kodları o tur için okumaz.", "task": "for i in range(5): if i==2: ___", "check": lambda c, o, i: "continue" in c, "solution": "continue", "hint": "continue."},
        {"msg": "**in:** Liste içinde gezinmeyi sağlayan aitlik kelimesidir. Listenin her elemanını sırayla bizim değişkenimize teslim eder.", "task": "for x ___ [1,2,3]: print(x)", "check": lambda c, o, i: "in" in c, "solution": "in", "hint": "in yaz."}
    ]},
    {"module_title": "5. Gruplama: Listeler (Veri Sepeti)", "exercises": [
        {"msg": "**Listeler:** Birden fazla veriyi tek kutuda (sepette) tutar. Python'da sepetin içindeki elemanları saymaya her zaman 0'dan başlarız!", "task": "L = [___, 20]", "check": lambda c, o, i: "10" in str(i.get('L','')), "solution": "10", "hint": "10 yaz."},
        {"msg": "**İndeksleme:** Listenin ilk elemanına `[0]` indeksiyle ulaşılır. Eğer 3. elemanı istiyorsan `[2]` yazmalısın. Bu kurala 'İndisleme' denir.", "task": "print(L[___])", "check": lambda c, o, i: "0" in c, "solution": "0", "hint": "0 yaz."},
        {"msg": "**.append():** Listenin sonuna yeni bir eleman ekler. Sepetini dinamik olarak büyütmek için bu metodu kullanırsın.", "task": "L.___ (30)", "check": lambda c, o, i: "append" in c, "solution": "append", "hint": "append metodu."},
        {"msg": "**len():** Listenin içindeki toplam eleman sayısını (uzunluğunu) verir. Kaç tane verimiz olduğunu ölçmek için hayatidir.", "task": "n = ___(L)", "check": lambda c, o, i: "len" in c, "solution": "len", "hint": "len yaz."},
        {"msg": "**.pop():** Listenin en sonundaki elemanı sepetten çıkarır ve siler. En son ekleneni atmak için kullanılır.", "task": "L.___()", "check": lambda c, o, i: "pop" in c, "solution": "pop", "hint": "pop."}
    ]},
    {"module_title": "6. Modülerlik: Fonksiyonlar ve Sözlükler", "exercises": [
        {"msg": "**def:** Fonksiyon (kod paketi) tanımlamak için kullanılır. 'Define' kelimesinden gelir. Tekrar eden kodları paketleyip her yerde çağırırsın.", "task": "___ pito(): print('Hi')", "check": lambda c, o, i: "def" in c, "solution": "def", "hint": "def yaz."},
        {"msg": "**Sözlük:** `{anahtar: değer}` yapısıyla çalışır. Tıpkı rehberdeki 'İsim' ve 'Numara' ilişkisi gibidir. Çok hızlı veri arar.", "task": "d = {'ad': '___'}", "check": lambda c, o, i: "Pito" in str(i.get('d',{})), "solution": "Pito", "hint": "Pito."},
        {"msg": "**Tuple:** Listeye benzer ama parantez `()` ile kurulur ve içeriği asla değiştirilemez. 'Mühürlü liste' diyebiliriz.", "task": "t = (___, 2)", "check": lambda c, o, i: "1" in str(i.get('t','')), "solution": "1", "hint": "1 yaz."},
        {"msg": "**.keys():** Sözlükteki tüm etiketleri (anahtarları) liste halinde bize sunar. Kimlerin kayıtlı olduğunu görmek gibidir.", "task": "print(d.___())", "check": lambda c, o, i: "keys" in c, "solution": "keys", "hint": "keys."},
        {"msg": "**return:** Fonksiyonun ürettiği sonucu dışarıya 'fırlatır'. Bu sayede fonksiyonun cevabını bir değişkene atayabilirsin.", "task": "def f(): ___ 5", "check": lambda c, o, i: "return" in c, "solution": "return", "hint": "return."}
    ]},
    {"module_title": "7. Nesneler: OOP Dünyası", "exercises": [
        {"msg": "**class:** Bir taslaktır. Ondan 'Nesneler' (Object) üretiriz. Sınıf bir fabrikadır, nesne ise o fabrikadan çıkan üründür.", "task": "___ Robot: pass", "check": lambda c, o, i: "class" in c, "solution": "class", "hint": "class yaz."},
        {"msg": "**Robot():** Kalıptan canlı bir nesne üretmek (Örneklem) için sınıf ismini parantezlerle çağırırız.", "task": "r = ___", "check": lambda c, o, i: "Robot" in str(i.get('r','')), "solution": "Robot()", "hint": "Robot()."},
        {"msg": "**Nokta (.):** Nesnelerin özelliklerine (rengi, hızı vb.) ulaşmak için nokta operatörünü kullanırız. Kimlik bilgileridir.", "task": "r.___ = 'Mavi'", "check": lambda c, o, i: "renk" in c, "solution": "renk", "hint": "renk."},
        {"msg": "**self:** Nesnenin kendisini temsil eden gizli parametredir. Sınıf içindeki metotlarda mutlaka ilk sırada olmalıdır.", "task": "def ses(___): pass", "check": lambda c, o, i: "self" in c, "solution": "self", "hint": "self."},
        {"msg": "**Method:** Nesneye bağlı çalışan fonksiyonlardır. Nesneye bir eylem yaptırmak için nokta ve parantez kullanılır.", "task": "r.___()", "check": lambda c, o, i: "s()" in c, "solution": "s()", "hint": "s()."}
    ]},
    {"module_title": "8. Kalıcılık: Dosya Yönetimi", "exercises": [
        {"msg": "**open():** Dosya açmak için kullanılır. **'w'** (write) yazma modudur. Dosya yoksa Python onu otomatik oluşturur.", "task": "f = ___('n.txt', '___')", "check": lambda c, o, i: "open" in c and "w" in c, "solution": "open, w", "hint": "open ve w."},
        {"msg": "**.write():** Veriyi dosyanın içine kalıcı olarak mühürler. Bellekteki bilgiyi diske kaydeder.", "task": "f.___('X')", "check": lambda c, o, i: "write" in c, "solution": "write", "hint": "write."},
        {"msg": "**'r':** Read (Okuma) modudur. Dosyayı sadece görmemizi sağlar, içinde değişiklik yapmamıza izin vermez.", "task": "f = open('t.txt', '___')", "check": lambda c, o, i: "r" in c, "solution": "r", "hint": "r koy."},
        {"msg": "**.read():** Dosyanın tüm içeriğini bir bütün olarak belleğe (RAM) çeker ve metin haline getirir.", "task": "print(f.___())", "check": lambda c, o, i: "read" in c, "solution": "read", "hint": "read."},
        {"msg": "**.close():** Dosyayı kapatmak sistem sağlığı için hayatidir! Kapatılmazsa veri kaybı veya kilitlenme olabilir.", "task": "f.___()", "check": lambda c, o, i: "close" in c, "solution": "close", "hint": "close."}
    ]}
]

# --- 7. ANA AKIŞ: GİRİŞ VE PANEL ---
with col_main:
    if st.session_state.is_logged_in:
    ...
    if not st.session_state.is_logged_in:
        ...
        with c2:
            st.markdown('<div class="pito-bubble" style="margin-top: 60px;">Merhaba Geleceğin Yazılımcısı!<br><br>Ben <b>Pito</b>. Python dünyasına adım atmaya hazır mısın? Numaranı gir ve akademiye katıl!</div>', unsafe_allow_html=True)
            ...
            if in_no and in_no.isdigit():
                ...
                if not user_data.empty:
                    ...
                    st.info(f"🔍 **{row['Öğrencinin Adı']}**, Hoş geldin! {'🎓 Mezuniyet mühürlendi.' if int(row['Mevcut Modül']) >= 8 else 'Kaldığın yerden devam edelim.'}")
    ...
    # --- KOD PANELİ KONTROL (ULTRA STABIL) ---
    if not is_review and st.session_state.fail_count < 4 and not st.session_state.exercise_passed:
        custom_input = ""
        if "input" in curr_ex['solution']: custom_input = st.text_input("📝 Girdi Kutusu:", key=f"inp_{curr_idx}").strip()
        
        code = st_ace(value=curr_ex.get('task',''), language="python", theme="monokai", font_size=18, height=220, key=f"ace_{curr_idx}_{st.session_state.fail_count}")
