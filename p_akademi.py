...
# Pito Resmi (Kırılmaması için kesinleşmiş URL fallback eklendi)
PITO_URL = "https://img.icons8.com/fluency/180/robot-viewer.png"
...
training_data = [
    {"module_title": "1. Giriş ve Çıktı", "exercises": [
        ...
        {"msg": "**# (Diyez)** işaretiyle başlayan satırlar Python tarafından okunmaz. Buna 'Yorum Satırı' denir. Hadi dene: Bir **yorum satırı** oluştur.", "task": "___ Bu bir yorumdur", "check": lambda c, o: "#" in c, "solution": "# Not"},
...
with col_main:
    ...
    with c_img: st.image(PITO_URL, width=140)
    with c_msg:
        st.info(f"##### 🗣️ Pito:\n{curr_ex['msg']}")
    ...
    def run_pito_code(c, user_input="Pito"):
        ...
        if "___" in c:
            return "⚠️ Boşluk Hatası: Lütfen önce kodun içindeki '___' alanlarını doldur!"
        
        old_stdout, new_stdout = sys.stdout, StringIO()
        sys.stdout = new_stdout
        try:
            # Akıllı Mock Girdisi: int() varsa 10 gönder (sonuç 11 çıksın diye)
    ...
    if is_locked:
        ...
        if st.button("🔍 Kontrol Et", use_container_width=True):
            ...
            if out.startswith("⚠️") or out.startswith("❌"):
                st.error(out)
            else:
