...
# --- 4. EKRAN DÜZENİ TANIMLAMA (LİDERLİK TABLOSU SAĞDA) ---
...
# --- 5. GİRİŞ EKRANI VE ANA AKIŞ ---
with col_app:
    if not st.session_state.is_logged_in:
        _, col_mid, _ = st.columns([1, 4, 1])
        with col_mid:
            st.markdown('<div class="pito-bubble">Merhaba Geleceğin Yazılımcısı! Ben <b>Pito</b>. Nusaybin Süleyman Bölünmez Anadolu Lisesi Python macerasına hazır mısın?</div>', unsafe_allow_html=True)
            show_pito_img(180)
    ...
    training_data = [
        {"module_title": "1. İletişim: print() ve Çıktı Dünyası", "exercises": [
            ...
            {"msg": "**Pito'nun Notu:** Sayılar (Integer), metinlerden farklıdır; tırnak gerektirmezler. Eğer tırnak koyarsan Python onu sayı değil, bir metin olarak görür ve üzerinde matematiksel işlemler yapamaz.\n\n**Görev:** Boşluğa tırnak kullanmadan sadece **100** sayısını yaz.", "task": "print(___)", "check": lambda c, o, i: "100" in o, "solution": "print(100)", "hint": "Rakamları tırnaksız yazmalısın. Eğer '100' yazarsan bu bir metin olur!"},
        ...
        {"module_title": "7. OOP: Nesne Tabanlı Dünya", "exercises": [
            ...
            {"msg": "**Pito'nun Notu:** Nesnelerin özellikleri (Attribute) nokta (`.`) yardımıyla atanır. Bu, o nesneye ait bir kimlik bilgisidir.\n\n**Görev:** r nesnesinin **renk** özelliğini 'Mavi' yapmak için boşluğa **renk** yaz.", "task": "class R: pass\nr = R()\nr.___ = 'Mavi'\nprint(r.renk)", "check": lambda c, o, i: "renk" in c, "solution": "r.renk = 'Mavi'", "hint": "Noktadan hemen sonra özellik ismini (renk) yerleştir."},
            {"msg": "**Pito'nun Notu:** `self` nesnenin kendisini temsil eden bir parametredir. Sınıf içindeki her 'eylem' (Metot) mutlaka `self` almalıdır.\n\n**Görev:** Metod parantezi içine **self** anahtarını yaz.", "task": "class R:\n def ses(___): print('Bip')", "check": lambda c, o, i: "self" in c, "solution": "def ses(self):", "hint": "Kendi anlamına gelen self kelimesini kullan."},
            {"msg": "**Pito'nun Notu:** Nesnenin bir eylemini (Metot) çalıştırmak için nesne isminden sonra nokta koyup metod ismini parantezli yazarız.\n\n**Görev:** r nesnesinin s() metodunu çalıştırmak için boşluğa **s()** yaz.", "task": "class R:\n def s(self): print('X')\nr = R()\nr.___()", "check": lambda c, o, i: "s()" in c, "solution": "r.s()", "hint": "Çağırılacak metot ismi s() fonksiyonudur."}
        ]},
        {"module_title": "8. Kalıcılık: Dosya Yönetimi", "exercises": [
            {"msg": "**Pito'nun Notu:** Program kapandığında veriler silinir. Saklamak için `open()` fonksiyonuyla dosya açarız. **'w'** (write) kipi yazmak içindir.\n\n**Görev:** n.txt dosyasını yazma modunda açmak için ilk boşluğa **open**, mod için **w** yaz.", "task": "f = ___('n.txt', '___')", "check": lambda c, o, i: "open" in c and "w" in c, "solution": "open('n.txt', 'w')", "hint": "Dosya açma fonksiyonu (open) ve yazma modunu (w) düşün."},
            {"msg": "**Pito'nun Notu:** `.write()` metodu veriyi dosyanın içine kalıcı olarak mühürler. Önceki içeriği siler ve yenisini ekler.\n\n**Görev:** Dosyaya 'X' yazmak için ilgili boşluğa **write** metodunu yaz.", "task": "f = open('t.txt', 'w')\nf.___('X')\nf.close()", "check": lambda c, o, i: "write" in c, "solution": "f.write('X')", "hint": "Yazma metodu olan write kelimesini yerleştir."},
    ...
    st.markdown(f"""<div class="quest-container"><div style="display: flex; justify-content: space-between; font-weight: bold; color: #3a7bd5;"><span>📍 {training_data[st.session_state.current_module]['module_title']}</span><span>🐍 Python Macerası: %{int(progress_perc)} Tamamlandı</span><span>🏆 {RUTBELER[min(sum(st.session_state.completed_modules), 8)]}</span></div><div class="quest-bar"><div class="quest-fill" style="width: {progress_perc}%;"></div></div></div>""", unsafe_allow_html=True)
    ...
    # KOD PANELİ
    if is_review_mode:
        pass # Çözüm zaten yukarıda sergilendi, editör kapalı kalsın
    else:
        if st.session_state.fail_count < 4 and not st.session_state.exercise_passed:
            ...
            if st.button("🔍 Kodumu Kontrol Et", use_container_width=True):
                if "___" in code: st.session_state.feedback_msg = "⚠️ Pito bekliyor: Lütfen önce boşluğu doldur!"; st.rerun()
                    ...
                    try:
                        # Mock Environment
                        mock_env = {"print": print, "input": lambda x: "10", "int": int, "str": str, "len": len, "open": open, "range": range, "s": 10, "L": [50, 60], "d":{'ad':'Pito'}, "t":(1,2), "Robot": lambda: None, "R": lambda: None}
