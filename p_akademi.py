...
training_data = [
    ...
    {"module_title": "5. Listeler", "exercises": [
        {"msg": "Listeler birden fazla veriyi tek bir kutuda tutan sıralı koleksiyonlardır. `[]` kullanılır. [10, 20] oluştur.", "task": "L = [___, 20]", "check": lambda c, o: "10" in c, "solution": "L=[10, 20]\nprint(L)"},
        {"msg": "Listelerin içindeki her verinin bir **indeksi** (sıra numarası) vardır. Çok önemli: Python'da sayma her zaman **0 (sıfır)**'dan başlar! Yani ilk eleman 0. indekstir. Elemana ulaşmak için `L[0]` yazımı kullanılır. Hadi dene: **L** listesinin ilk elemanına (0. indeks) eriş.", "task": "L=[5,6]\nprint(L[___])", "check": lambda c, o: "5" in o, "solution": "L=[5,6]\nprint(L[0])"},
    ...
    {"module_title": "6. Fonksiyonlar ve Gelişmiş Veriler", "exercises": [
        ...
        {"msg": "**Tuple (Demet)**, listelere çok benzer ancak en büyük farkı **değiştirilemez** (immutable) olmasıdır. Yani bir kez oluşturduğunuzda içindeki bir veriyi silemez veya güncelleyemezsiniz. Listeler için `[]` kullanılırken, Demetler için **`()` (normal parantez)** kullanılır. Hadi dene: İçinde **1** ve **2** sayıları olan bir demet oluştur.", "task": "t = (___, 2)\nprint(t)", "check": lambda c, o: "1" in c, "solution": "t = (1, 2)\nprint(t)"},
        ...
        {"msg": "keys() metodu sözlükteki tüm anahtar başlıklarını sunar.", "task": "d={'a':1}\nprint(d.___())", "check": lambda c, o: "keys" in c, "solution": "d={'a':1}\nprint(d.keys())"},
    ...
    {"module_title": "7. OOP (Sınıf ve Metotlar)", "exercises": [
        ...
        {"msg": "**Nesneler (Object)**, sınıflardan (taslaklardan) üretilen gerçek somut örneklerdir. Bir mimari plan (sınıf) kullanarak inşa edilen gerçek bir ev bir nesnedir. Nesne oluşturmak için sınıf adını ve yanına parantezlerini yazarız: `nesne = SınıfAdı()`. Hadi dene: **R** sınıfını kullanarak **p** adında bir nesne oluştur.", "task": "class R: pass\np = ___()", "check": lambda c, o: "R()" in c, "solution": "class R: pass\np = R()"},
...
with col_main:
    ...
    if st.session_state.db_module >= 8:
        ...
        with c1:
            if st.button("🔄 Eğitimi Tekrar Al (Sıfırla)"):
                # TÜM KOORDİNATLAR TAM SIFIRLANIYOR
    ...
    # ACE EDITOR - AUTO UPDATE AKTİF (APPLY BUTONU KALDIRILDI)
