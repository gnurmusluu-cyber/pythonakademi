...
def show_pito_gif(width=450):
    ...
    if gif_path.exists():
        ...
        t = time.time() # Cache kırmak için
        st.markdown(f'<div style="display: flex; justify-content: center; margin-bottom: 20px;"><img src="data:image/gif;base64,{b64}?t={t}" width="{width}" style="border-radius: 20px;"></div>', unsafe_allow_html=True)
    else:
...
with col_app:
        ...
        if in_no:
            if not in_no.isdigit(): st.error("⚠️ Sadece rakam giriniz!")
                ...
                if not user_data.empty:
                    ...
                    st.warning("🌟 Yeni bir kayıt oluşturmalısın!")
    ...
    training_data = [
        ...
        {"module_title": "2. Hafıza: Değişkenler ve input()", "exercises": [
            ...
            {"msg": "**Pito'nun Notu:** Metin (String) verilerini hafızaya alırken tırnak şarttır. Değişken isimlerinde rakamla başlamamaya ve boşluk kullanmamaya dikkat etmelisin.\n\n**GÖREV:** `isim` değişkenine **'Pito'** metnini ata. Metni tırnak içinde yazmayı unutma!", "task": "isim = '___'", "check": lambda c, o, i: "Pito" in str(i.get('isim', '')), "solution": "isim = 'Pito'", "hint": "Tırnaklar arasına Pito yaz."},
            ...
            {"msg": "**Pito'nun Notu:** Sayıları metne çevirmemiz gerektiğinde (Buna 'Casting' diyoruz) `str()` fonksiyonunu kullanırız. Bu, sayıları ve metinleri birleştirirken hata almanı önler.\n\n**GÖREV:** 10 sayısını metne çeviren **str** fonksiyonunu boşluğa yerleştir.", "task": "print(___(10))", "check": lambda c, o, i: "str" in c, "solution": "print(str(10))", "hint": "str(sayı) formunu kullan."},
        ...
        {"module_title": "8. Kalıcılık: Dosya Yönetimi", "exercises": [
            {"msg": "**open():** Bilgileri saklamak için `open()` kullanılır. **'w'** (write) yazma modudur.\n\n**GÖREV:** Yazma modunda açmak için **open** ve **w** yaz.", "task": "f = ___('n.txt', '___')", "check": lambda c, o, i: "open" in c and "w" in c, "solution": "f = open('n.txt', 'w')", "hint": "open ve w."},
            {"msg": "**.write():** `.write()` metodu veriyi dosyanın içine kalıcı olarak mühürler.\n\n**GÖREV:** Dosyaya 'X' yazmak için boşluğa **write** metodunu yaz.", "task": "f = open('t.txt', 'w')\nf.___('X')\nf.close()", "check": lambda c, o, i: "write" in c, "solution": "f = open('t.txt', 'w')\nf.write('X')\nf.close()", "hint": "write yaz."},
    ...
    # GÖRSEL HİYERARŞİ: Pito (1.5) ve Konuşma Balonu (3.5)
