...
# --- 1. SİBER-ZIRH: GÖZ DOSTU YÜKSEK KONTRAST CSS ---
st.set_page_config(
...
st.markdown("""
    ...
    /* 2. PARLAYAN BAŞLIK (YUMUŞAK ADAÇAYI GEÇİŞ) */
    .academy-title { 
        font-size: 3.8em !important; font-weight: 900 !important;
        background: linear-gradient(90deg, #A8E6CF, #00CCFF) !important; 
        -webkit-background-clip: text !important; -webkit-text-fill-color: transparent !important; 
        margin-bottom: 30px !important;
    }

    /* 3. MODÜL BAŞLIĞI (ADAÇAYI ÜSTÜNE SİYAH - KESİN OKUNABİLİRLİK) */
    .hero-panel { 
    ...
    .hero-panel p { color: #1B4D3E !important; font-weight: 700 !important; margin: 0 !important; }

    /* 4. EXPANDER BAŞLIKLARI (ADAÇAYI ÜSTÜNE SİYAH) */
    ...
    [data-testid="stWidgetLabel"] p { color: #A8E6CF !important; font-weight: 900; font-size: 1.25em; }
    textarea, input { color: #00CCFF !important; background-color: #000000 !important; border: 1px solid #A8E6CF !important; }
    ...
    /* 9. BUTONLAR VE İLERLEME */
    .stButton>button { border-radius: 12px; background-color: #A8E6CF !important; color: black !important; font-weight: 800; height: 3.5em; width: 100%; }
    div.stProgress > div > div > div > div { background-color: #A8E6CF !important; }
    .progress-label { font-size: 0.85em; color: #A8E6CF !important; font-weight: 900; display: flex; justify-content: space-between; margin-bottom: 5px; }
    .pito-notu { background-color: #1E1E2F !important; border-radius: 12px; padding: 22px; border-left: 6px solid #A8E6CF; color: #E0E0E0; font-style: italic; }
    ...
    .stTabs [data-baseweb="tab"] p { color: #A8E6CF !important; font-weight: bold !important; }
    .stTabs [aria-selected="true"] { background-color: #A8E6CF !important; border-radius: 8px; }
...
def liderlik_tablosu_goster(user_sinif=None):
    st.markdown("<h3 style='text-align:center; color:#A8E6CF;'>🏆 ONUR KÜRSÜSÜ</h3>", unsafe_allow_html=True)
...
if st.session_state.user is None:
    ...
    if m_idx >= total_m: # --- MEZUNİYET ---
        ...
        with cl:
            ...
            st.markdown(f'<div style="background:#161b22; padding:12px; border-radius:12px; margin-bottom:15px;">💎 {p_xp} XP | ⚠️ Hata: {st.session_state.error_count}/4</div>', unsafe_allow_html=True)
            
            cp1, cp2 = st.columns([1, 2])
            with cp1: pito_gorseli_yukle(st.session_state.pito_mod, size=180)
            with cp2:
                if st.session_state.error_count == 1: st.error("🚨 Pito: Bu senin 1. hatan hocam! Dikkatli ol."); st.session_state.pito_mod = "hata"
                elif st.session_state.error_count == 2: st.error("🚨 Pito: Bu senin 2. hatan! Kodu tekrar gözden geçir."); st.session_state.pito_mod = "hata"
                elif st.session_state.error_count == 3: st.warning(f"💡 İpucu: {egz['ipucu']}"); st.session_state.pito_mod = "hata"
                elif st.session_state.error_count >= 4: st.error("🚫 Maksimum hata! Çözümü incele."); st.session_state.pito_mod = "dusunuyor"
                st.markdown(f"<div class='pito-notu'>💬 <b>Pito:</b> {u['ad_soyad'].split()[0]}, Python seninle!</div>", unsafe_allow_html=True)
            ...
            if not st.session_state.cevap_dogru and st.session_state.error_count < 4:
                ...
                with st.expander("📖 PİTO'NUN KESİN ÇÖZÜMÜ", expanded=True):
                    ...
                    st.markdown(f"<div class='console-box'>💻 Çıktı: {egz['beklenen_cikti']}</div>", unsafe_allow_html=True)
